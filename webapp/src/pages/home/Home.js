...
class Home extends Component {
        ...
        if (response.events) {
          this.setState({
            upcomingEvents: response.events.filter(
              (e) =>
                e.event_type === "EVENT" &&
                (e.is_event_opening || e.is_event_open)
            ),
            awards: response.events.filter(
              (e) =>
                e.event_type === "AWARD" &&
                (e.is_event_opening || e.is_event_open)
            ),
            calls: response.events.filter(
              (e) =>
                e.event_type === "CALL" &&
                (e.is_event_opening || e.is_event_open)
            ),
              ...
              return (
                <div class="event" key={e.key}>
                  <div class="event-info">
                    <h5>
                      <NavLink to={`/${e.key}`}>{e.description}</NavLink>
                    </h5>
  ...
  render() {
    ...
    if (
      ...
      this.state.organisation.name === "AI4D Africa" &&
    ...
    return (
      <div>
        <div>
          <img
            src={this.state.organisation && require("../../images/" + logo)}
        ...
        {!this.props.user && (
          <div className="text-center">
            {this.state.organisation && (
              <h2 className="Blurb text-center">
                {t("Welcome to") + " "} {this.state.organisation.system_name}
              </h2>
            )}
            <p className="text-center">
              <NavLink to="/createAccount" id="nav-signup">
                {t("Sign Up")}
              </NavLink>{" "}
              {t(
                "for an account in order to apply for an event, award or call for proposals"
              )}
              .{" "}
              <NavLink id="nav-login" to="/login">
                {t("Sign In")}
              </NavLink>{" "}
              {t("if you already have one")}.
            </p>
          </div>
        )}
