...
const ApplicationForm = (props) => {
  ...
  const handleSave = async () => {
    ...
    if (!isInCreateMode) {
      if (!isSaved) {
        const res = await updateApplicationForm(
          id,
          eventId,
          isOpen,
          nominate,
  ...
  const EventMeta = ({ handleCheckChanged, dateFormat, evnt }) => {
    return (
      <>
        <div className="nominations-desc">
          ...
          <span htmlFor="nomination-chck" className="nomination-info">
            {t(
              "Allow candidates to nominate others using this application form" +
