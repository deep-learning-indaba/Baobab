...
const Question = forwardRef(
  (
    ...
    const max_num_referral =
      inputs.options &&
      inputs.options[lang] &&
      inputs.options[lang].max_num_referral;
    const min_num_referral =
      inputs.options &&
      inputs.options[lang] &&
      inputs.options[lang].min_num_referral;
    const validation_regex =
      inputs.validation_regex && inputs.validation_regex[lang];
    ...
    useEffect(() => {
      if (
        type === "reference" &&
        (!parseInt(max_num_referral) || !parseInt(min_num_referral))
      ) {
        ...
        (type === "multi-choice" || type === "multi-checkbox") &&
        opts &&
        !opts.length
      ) {
    ...
    useEffect(() => {
      if (
        errorMessage ||
        ((type === "multi-choice" || type === "multi-checkbox") && emptyOptions)
      ) {
    ...
    const handleOkDelete = () => {
      ...
      const newQuestions = questionsWithNoDependency.filter(
        (q) => q.id !== inputs.id
      );
      const orderedQuestions = newQuestions.map((q, i) => ({
        ...q,
        order: i + 1,
      }));
    ...
    const handlereferralsChange = (prop) => (e) => {
      ...
      let max;
      if (prop === "min") {
        max = inputs.options[lang] && inputs.options[lang].max_num_referral;
        min = e.target.value;
      }
      if (prop === "max") {
        min = inputs.options[lang] && inputs.options[lang].min_num_referral;
        max = e.target.value;
      }
      const updatedSections = sections.map((s) => {
        if (s.id === sectionId) {
    ...
    const dependencyOptions = optionz.filter(
      (e) => e.order < inputs.order || !e.order
    );
          ...
          <div className="headline-description">
              ...
              <div className="referals-wrapper">
                ...
                <div className="min-wrapper">
                  ...
                  <input
                    ...
                    min={0}
                    placeholder={
                      inputs.options["en"].max_num_referral ||
            ...
            {withExtention.includes(inputs.type) && (
              <div className="extensions-wrapper">
                <label htmlFor="extensions" className="extension-label">
                ...
                <input
                  ...
                  placeholder={
                    ...
                    t(
                      "Please enter a list of file extensions, each starting with a period and separated with commas. E.g .csv,.xls"
                    )
                  }
                  className="question-inputs question-headline"
                  value={
                    (inputs.options[lang] &&
                      inputs.options[lang].accept &&
              ...
              <div className="question-footer">
                ...
                {inputs.type !== "information" && (
                  <div className="require-chckbox">
                    ...
                    />
                    <label htmlFor={`required_${inputs.id}`}>
                      {t("Required")}
                    </label>
                  </div>
                )}
            ...
            <div
              ...
              aria-expanded="false"
            >
              <i className="fa fa-ellipsis-v fa-lg fa-dropdown fa-question-toogle"></i>
            </div>
            <div className="dropdown-menu" aria-labelledby="toggleTitle">
              ...
              {hasKey && (
                ...
                >
                  <span className="check-icon">
                    {isKeyOn && <i className="fas fa-check fa-validation"></i>}
                  </span>
                  {t("Key")}
                </button>
              )}
