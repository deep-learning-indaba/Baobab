...
export const langObject = (langs, value) =>
  langs &&
  langs.reduce(
    (obj, item) =>
      Object.assign(obj, {
 ...
 */
export const AnimateSections = ({ children, applyTransition }) => {
...
export const drop = ({
  ...
  const dropSection = elements.find((e) => e.id === event.currentTarget.id);
...
export const Modal = ({
  ...
  const dependentQestions =
    questions &&
    questions.filter(
      (q) =>
        (q.depends_on_question_id &&
          inputs.backendId &&
          q.depends_on_question_id === inputs.backendId) ||
        (q.depends_on_question_id &&
          inputs.surrogate_id &&
  ...
  if (questionsInSection && sectionDependency) {
    message =
      "One or more questions in this section is a dependency of other sections. Are you sure you still want to delete it?";
  } else if (
    (dependentSections && dependentSections.length) ||
...
export const StageModal = React.memo(
  ({
    ...
    const message =
      "Some Changes have not been saved. Are you sure you want to leave without saving them?";
...
export const handleMove = ({
  ...
  if ((isUp && index !== 0) || (!isUp && index !== elements.length - 1)) {
    [nEl[isUp ? index - 1 : index + 1], nEl[index]] = [
      nEl[index],
      nEl[isUp ? index - 1 : index + 1],
    ];
...
export const SelectQuestion = ({
  options,
  t,
  handleAppFormQuestionSelect,
  inputs,
}) => {
  const selectedQuestion = options.find(
    (o) => parseInt(o.value) === inputs.question_id
  );
  return (
    <ReactSelect
      options={options}
...
export const Dependency = ({
  ...
  const dependency = options.find(
    (o) => parseInt(o.value) === inputs.depends_on_question_id
  );
    ...
    <div className="dependency-wrapper">
      <ReactSelect
        ...
        styles={{
          control: (base, state) => ({
            ...
            border: "none",
            transition:
              state.isFocused && "color,background-color 1.5s ease-out",
            background: state.isFocused && "lightgray",
            color: "#fff",
          }),
          option: (base, state) => ({
      ...
      {dependentQuestion &&
        ...
        dependentQuestion.options[lang].map((option, i) => (
          <div className="dependency-options-wrapper" key={option.id}>
            <input
              id={option.id}
              type="checkbox"
              className="single-choice-check"
              checked={
                inputs.show_for_values[lang] &&
...
export const Validation = ({ t, options, inputs, lang, handleChange }) => {
  ...
  return (
    <div className="validation-wrapper">
      <span className="regex-label">{t("Regular Expressions")}</span>
      <ReactSelect
        options={options}
      ...
      {isFriendlyMode ? (
        <>
          <div className="min-wrapper">
            <label htmlFor="minimum" className="validation-label">
              {t("Min")}
            </label>
            <input
              id="minimum"
          ...
          <div className="min-wrapper">
            <label htmlFor="maximum" className="validation-label">
              {t("Max")}
            </label>
            <input
              id="maximum"
        ...
        <div className="min-wrapper">
          <label htmlFor="regex" className="validation-label">
            {t("Enter Your Regex")}
          </label>
          <input
            name="regex"
            ...
            placeholder={inputs.validation_regex["en"] || t("Enter Your Regex")}
      ...
      <div className="min-wrapper">
        <label htmlFor="validation" className="validation-label">
          {t("Validation Text")}
        </label>
        <input
          name="validation"
          ...
          s = {
            ...
            questions: s.questions.map((q) => {
              if (q.id === inputs.id) {
                ...
                q = {
                  ...q,
                  show_for_values: { ...sfv, [lang]: !q.show_for_values[lang] },
                };
              }
              return q;
            }),
...
export const Stages = ({
  ...
  return (
    <div className="review-stage-wrapper">
      <div className="review-stage-label">{t("Stages")}</div>
      <div className="review-stage-btns">
        <div className="review-stages">
          {Array.from(Array(totalStages), (_, index) => index + 1).map(
            (s, i) => {
              return (
                <input
                  type="button"
