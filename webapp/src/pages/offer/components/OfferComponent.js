...
class Offer extends Component {
              ...
              () => {
                ...
                if (candidate_response && this.props.event) {
                  this.props.history.push(
                    `/${this.props.event.key}/registration`
                  );
                }
              }
            );
  ...
  displayOfferResponse = () => {
    ...
    let responded_date =
      offer.responded_at !== undefined
        ? offer.responded_at.substring(0, 10)
        : "-date-";
      ...
      <div className="container">
        <p className="h5 pt-5">
          {offer.candidate_response && (
            <span>You accepted the following offer on {responded_date}.</span>
          )}
          {!offer.candidate_response && (
            <span class="text-danger">
              You rejected your offer for a spot at {event ? event.name : ""} on{" "}
              {responded_date} for the following reason:
              <br />
              <br />
        ...
        {offer.candidate_response && (
          <div className="white-background card form mt-5">
            {this.row(
              "Offer date",
              offer.offer_date !== undefined
                ? offer.offer_date.substring(0, 10)
                : "-date-"
            )}
            {this.row(
              "Offer expiry date",
              offer.expiry_date !== undefined
                ? offer.expiry_date.substring(0, 10)
                : "-date-"
            )}
            {this.row(
              "Registration fee",
              offer.payment_required
                ? "Payment of 350 USD Required to confirm your place"
                : "Fee Waived"
            )}

            {this.props.event &&
              this.props.event.travel_grant &&
              this.row(
                "Travel",
                offer.accepted_travel_award
                  ? "Your travel to and from Nairobi will be arranged by the Indaba"
                  : "You are responsible for your own travel to and from Nairobi."
              )}
            {this.props.event &&
              this.props.event.travel_grant &&
              this.row(
                "Accommodation",
                offer.accepted_accommodation_award
                  ? "Your accommodation will be covered by the Indaba in a shared hostel from the 25th to 31st August"
                  : "You are responsible for your own accommodation in Nairobi."
              )}
          </div>
        )}
          ...
          <div className="row mt-4">
            ...
            <div className="col">
              <NavLink
                ...
                to={`/${this.props.event.key}/registration`}
              >
  ...
  renderTravelAward = (offer, accepted_travel_award) => {
    return (
      <div class="row mb-4">
        <div class="col-md-3 font-weight-bold pr-2" align="center">
          Travel:
        </div>
        <div class="col-md-6" align="left">
          ...
          {offer && offer.travel_award && !accepted_travel_award && (
            <span class="text-danger">
              You have chosen to reject the travel award - you will be
              responsible for your own travel to and from Nairobi!
            </span>
          )}
          {offer &&
            offer.requested_travel &&
            !offer.travel_award &&
      ...
      <div class="row mb-2">
        ...
        <div class="col-md-6" align="left">
          ...
          {offer &&
            ...
            !accepted_accommodation_award && (
              <span class="text-danger">
                You have chosen to reject the accommodation award - you will be
                responsible for your own accommodation in Nairobi during the
                Indaba!
              </span>
            )}
          {offer &&
            offer.requested_accommodation &&
            !offer.accommodation_award && (
              <span>
                Unfortunately we are unable to grant you the accomodation award
                you requested in your application. We do however have reasonably
                priced accommodation available in shared hostel rooms on campus,
                available on a first come first serve basis. Please see{" "}
                <a href="www.deeplearningindaba.com/accommodation-2019">here</a>{" "}
                for more details
              </span>
            )}
          {offer &&
        ...
        <div class="col-md-3">
          {offer.accommodation_award && (
            <div class="form-check accommodation-container">
              ...
              />
              <label class="form-check-label" for="CheckAccommodation">
                I accept the accommodation award.
              </label>
            </div>
          )}
        </div>
        ...
        {offer.candidate_response !== null ? (
          ...
          <div className="container">
            <p className="h5 pt-5">
              ...
              {this.props.event ? this.props.event.name : ""}. Please see the
              details of this offer below{" "}
            </p>
            ...
            <form class="form pt-2 ">
              <div className="white-background card form">
                ...
                {this.props.event &&
                  this.props.event.travel_grant &&
                  this.renderTravelAward(offer, accepted_travel_award)}
                {this.props.event &&
                  this.props.event.travel_grant &&
                  this.renderAccommodationAward(
                    offer,
                    accepted_accommodation_award
                  )}
                ...
                <div class="row mb-3">
                  ...
                  <div class="col-md-6" align="left">
                    {offer &&
                      offer.payment_required &&
                      "In order to confirm your place, you will be liable for a 350 USD registration fee."}
                    {offer &&
                      !offer.payment_required &&
  ...
  componentDidMount() {
    applicationFormService
      .getResponse(this.props.event ? this.props.event.id : 0)
      .then((results) => {
        if (
          results.response &&
          results.response.length > 0 &&
          results.response[0].is_submitted &&
          !results.response[0].is_withdrawn
        ) {
          this.setState({
  ...
  getOffer = () => {
    ...
    offerServices
      .getOffer(this.props.event ? this.props.event.id : 0)
      .then((result) => {
        if (result.error && result.statusCode === 404) {
          ...
          this.setState({
            ...
            error: result.error,
            accepted_accommodation_award:
              result.offer.accepted_accommodation_award === null
                ? result.offer.accommodation_award
                : result.offer.accepted_accommodation_award,
            accepted_travel_award:
              result.offer.accepted_travel_award === null
                ? result.offer.travel_award
                : result.offer.accepted_travel_award,
          });
        }
      });
  };
