...
class ReviewAssignmentComponent extends Component {
  ...
  componentDidMount() {
    reviewService
      ...
      .then((result) => {
        ...
        return reviewService.getReviewSummary(
  ...
  assignReviewers = (email, toAssign) => {
    ...
    reviewService
      ...
      .then((result) => {
        ...
        return reviewService.getReviewAssignments(
          this.props.event ? this.props.event.id : 0
        );
      })
      .then(
    ...
    return (
      <div
        ...
        onBlur={(e) => {
          ...
          reviewers[cellInfo.index][cellInfo.column.id] = parseInt(
            e.target.innerHTML
          );
      ...
      <button
        ...
        onClick={() =>
          this.assignReviewers(
            cellInfo.row.email,
            cellInfo.row.reviews_to_assign
          )
        }
  ...
  render() {
    const { loading, reviewers, error, newReviewerEmail, reviewSummary } =
      this.state;
    ...
    return (
      <div className={"review-padding"}>
        {error && <div class="alert alert-danger alert-container">{error}</div>}

        {reviewSummary && (
          <span className="review-padding">
            {t("Total Unallocated Reviews:")}{" "}
            {reviewSummary.reviews_unallocated}
          </span>
        )}

        <ReactTable data={reviewers} columns={columns} minRows={0} />
        <br />
        <div>
          <FormTextBox
            id={"newReviewEmail"}
            name={"newReviewEmail"}
            label={t(
              "Add new reviewer's email (they must already have an account)"
            )}
          ...
          <button
            ...
            onClick={() => {
              this.assignReviewers(this.state.newReviewerEmail, 0);
            }}
          >
            {t("Add")}
          </button>
