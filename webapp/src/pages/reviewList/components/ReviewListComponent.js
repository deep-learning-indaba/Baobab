...
class ReviewListComponent extends Component {
  ...
  toggleHideSubmitted = () => {
    this.setState({
      reviewList: !this.state.hideSubmitted
        ? this.state.originalReviewList.filter((r) => !r.submitted)
        : this.state.originalReviewList,
  ...
  render() {
    ...
    const statusCell = (props) => {
      if (props.original.started && props.original.submitted) {
        return (
          <div>
            <span class="badge badge-success">{this.props.t("Submitted")}</span>{" "}
            <span className="submitted-date">
              {new Date(props.original.submitted).toLocaleString("en-GB")}
            </span>
          </div>
        );
      }
      if (props.original.started) {
        return (
          <span class="badge badge-warning">{this.props.t("In Progress")}</span>
        );
      }
      return (
        <span class="badge badge-secondary">{this.props.t("Not Started")}</span>
      );
    };
    ...
    return (
      <div className="review-list-container">
        ...
        {numReviews > 0 && numReviews > numCompleted && (
          <p class="summary">
            <Trans>
              You have {{ numReviews }} reviews assigned, of which{" "}
              {{ numCompleted }} are completed
            </Trans>
          </p>
        )}
        ...
        <div class="checkbox-top">
          <input
            onClick={(e) => this.toggleHideSubmitted()}
            className="form-check-input input"
            type="checkbox"
            value={hideSubmitted}
            id="defaultCheck1"
          />
          <label id="label" class="label-top" for="defaultCheck1">
            {t("Hide Completed")}
          </label>
        </div>
