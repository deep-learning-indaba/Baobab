...
class ReviewQuestionComponent extends Component {
  ...
  renderHeader = (model) => {
    ...
    if (model.question.type === SUB_HEADING) {
      ...
      model.question.type === INFORMATION ||
      model.question.type === FILE ||
      model.question.type === MULTI_FILE ||
      model.question.type === HEADING
    ) {
  ...
  render() {
    ...
    if (
      this.props.model.question.type === INFORMATION ||
      this.props.model.question.type === FILE ||
      this.props.model.question.type === MULTI_FILE ||
      this.props.model.question.type === HEADING
    ) {
      className = className + " information";
    } else if (
      this.props.model.question.type !== SECTION_DIVIDER &&
      this.props.model.question.type !== SUB_HEADING
    ) {
    ...
    return (
      <div className={className}>
        ...
        {this.props.model.question.description && (
          <ReactMarkdown
            remarkPlugins={[gfm]}
            source={this.props.model.question.description}
...
class ReviewForm extends Component {
  ...
  processResponse = (response) => {
    ...
    if (
      !response.form.review_response ||
      (response.form.review_response.id === 0 &&
        !response.form.review_response.scores)
    ) {
    ...
    if (response.form) {
      ...
      questionModels = response.form.review_form.review_sections
        .map((s) => {
          return {
            headline: s.headline,
            ...
            id: s.id,
            order: s.order,
            questions: s.review_questions
              .map((q) => {
                let score = null;
                if (response.form.review_response) {
                  score = response.form.review_response.scores.find(
                    (a) => a.review_question_id === q.id
                  );
                }
                return {
                  question: q,
                  answer: response.form.response.answers.find(
    ...
    const totalScore = questionModels
      ? this.computeTotalScore(questionModels)
      : 0;
  ...
  computeTotalScore = (questionModels) => {
    return questionModels
      ...
      .reduce(
        (acc, q) =>
          acc +
          (q.question.weight > 0 && q.score && parseFloat(q.score.value)
            ? parseFloat(q.score.value)
            : 0),
        0
      );
  };
  ...
  isValidated = (checkRequired) => {
    ...
    const isValid = !validatedModels
      .map((s) => s.questions)
      .some((v) => v.validationError);
  ...
  save = () => {
    const scores = this.state.questionModels
      .flatMap((s) => s.questions)
      .filter((qm) => qm.score)
      .map((qm) => qm.score);
  ...
  submit = () => {
    let scores = this.state.questionModels
      .flatMap((s) => s.questions)
      .filter((qm) => qm.score)
      .map((qm) => qm.score);
              ...
              if (response.error) {
                ...
                if (this.props.match.params && this.props.match.params.id > 0) {
                  this.props.history.push(
                    `/${this.props.event.key}/reviewlist`
                  );
                } else {
                  this.loadForm();
                }
  ...
  render() {
      ...
      return (
        <div class="review-form-container">
          <div class="alert alert-success alert-container">
            <p class="complete-title">{t("All Done!")}</p>
            <br />
            {t(
    ...
    return (
      <div class="review-form-container">
        {questionModels &&
          questionModels.map((section) => (
            <div className="card review-section" key={"s_" + section.id}>
              {section.headline && (
                <h3 className="section-headline card-title">
                  {section.headline}
                </h3>
              )}
              {section.description && (
        ...
        <div>
          {t("Response ID")}:{" "}
          <span className="font-weight-bold">{form.response.id}</span> -{" "}
          {t(
            "Please quote this in any correspondence with admins outside of the system."
          )}
        </div>
        ...
        <div className="floating-bar">
          ...
          {this.state.saveValidationFailed && (
            <span className="save-validation-failed text-danger">
              ...
              {this.props.t("Please fix validation errors before saving")}
            </span>
          )}

          {this.state.saveSuccess && (
            <span>
              <span className="save-validation-failed text-success">
                {this.props.t("Saved")}
              </span>
              <span className="return-to-list">
                <Link to={`/${this.props.event.key}/reviewlist`}>
                  {this.props.t("Return to review list")}...
                </Link>
              </span>
            </span>
          )}
        ...
        <div class="buttons">
          {currentSkip > 0 && (
            <button
              ...
              className={
                "btn btn-secondary " + (form.review_response ? "hidden" : "")
              }
        ...
        {!editMode && (
          <div class="alert alert-info">
            ...
            <Trans i18nKey="reviewsRemaining">
              You have {{ reviewsRemainingCount }} reviews remaining
            </Trans>
          </div>
        )}
        ...
        >
          <div class="flagModal">
            <p>
              {t(
                "If reviewing this response revealed an issue that should be considered if this candidate were accepted, please describe it below."
              )}
            </p>
            <textarea
