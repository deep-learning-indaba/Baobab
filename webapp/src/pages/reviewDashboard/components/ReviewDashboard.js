...
class ReviewListComponent extends Component {
  ...
  processReviewList = (reviewList) => {
      ...
      row.identifiers.forEach((i) => {
        if (!columns.some((c) => c.Header === i.headline)) {
          columns.push({
            ...
            Header: i.headline,
            accessor: (r) => {
              const identifier = r.identifiers.find(
                (x) => x.headline === i.headline
              );
    ...
    reviewList.forEach((row) => {
      row.scores.forEach((i) => {
        const headline = `${i.headline ? i.headline + "; " : ""}${
          ...
          columns.push({
            ...
            accessor: (r) => {
              const score = r.scores.find(
                (s) => s.review_question_id === i.review_question_id
              );
  ...
  loadReviewList = (mode) => {
    ...
    const promise =
      mode === "details"
        ? reviewService.getReviewDetails(eventId)
        : reviewService.getReviewSummaryList(eventId);
    ...
    promise.then((response) => {
      ...
      const sortedReviewList = result.reviewList.sort(
        (a, b) => a.response_id - b.response_id
      );
      this.setState({
        isLoading: false,
  ...
  exportToCSV() {
    ...
    const output = [columns.map((c) => c.Header)];
    ...
    const filename =
      `review_${this.state.mode}_` +
      new Date().toDateString().split(" ").join("_") +
      ".csv";
  ...
  render() {
    const { error, isLoading, infoColumns, reviewList } = this.state;
    ...
    const columns = [
      ...
      },
      {
        ...
        id: "candidate",
        accessor: (r) =>
          `${r.response_user_title} ${r.response_user_firstname} ${r.response_user_lastname}`,
        filterable: false,
      },
    ];
    ...
    if (this.state.mode === "details") {
      columns.push({
        ...
        accessor: (r) =>
          `${r.reviewer_user_title} ${r.reviewer_user_firstname} ${r.reviewer_user_lastname}`,
        filterable: false,
      });
    }
...
class ReviewAssignmentComponent extends Component {
  ...
  componentDidMount() {
    reviewService
      .getReviewAssignments(this.props.event ? this.props.event.id : 0)
      .then((result) => {
        this.setState({
          loading: false,
...
const ReviewAssignment = withRouter(
  withTranslation()(ReviewAssignmentComponent)
);
        ...
        <h2 className="title">
          ...
          {stage && (
            <span className="pull-right text-primary">
              <h2 className="stage-info text-center">
