...
class ResponsePage extends Component {
  ...
  postResponseTag(tagId, responseId, eventId) {
    ...
    responsesService
      .tagResponse(responseId, tagId, eventId)
      .then((response) => {
        if (response.status == 201) {
          const getTag = this.state.tagList.find(
            (t) => t.id === response.responseTag.tag_id
          );
    ...
    responsesService
      ...
      .then((response) => {
        if (response.status == 200) {
          const index = updateApplicationData.tags.findIndex(
            (t) => t.id === tagToRemove
          );
          if (index >= 0) updateApplicationData.tags.splice(index, 1);
  ...
  renderTags() {
    ...
    if (data) {
      let tags = data.tags
        ? data.tags.map((tag) => {
            return (
              <span key={tag.id} className="btn badge badge-info">
                {tag.name}
                <i
  ...
  renderReviewerModal() {
    return (
      <ReviewModal
        ...
        reviewers={this.state.reviewers.filter(
          (r) =>
          ...
          <div className="headings-lower">
            <div className="user-details">
              ...
              <div className="tags">
                {this.renderTags()}
                <span
                  onClick={(e) => this.toggleTags(tagMenu)}
                  className={
                    (tagMenu && tagList.length) ||
                    (tagMenu && eventLanguages.length)
              ...
              >
                {tagList &&
                  tagList.map((val) => {
                    return (
                      <div className="tag-item" key={val.id}>
                        <button
                          onClick={(e) =>
                            this.postResponseTag(
                              val.id,
