...
const ReviewForm = (props) => {
  ...
  useEffect(() => {
    ...
    reviewService.getReviewStage(eventId).then((res) => {
      setStage({
        loading: false,
        stage: res.data,
        error: res.error,
    ...
    applicationFormService.getDetailsForEvent(eventId).then((res) => {
  ...
  useEffect(() => {
    ...
    if (currentStage && !event.loading) {
      reviewService.getReviewFormDetails(eventId, currentStage).then((res) => {
        if (res.data) {
  ...
  const handleSave = async () => {
    ...
    const { id, eventId, isOpen, applicationFormId, stage, deadline, active } =
      formDetails;
    if (!isInCreateMode) {
      if (!isSaved) {
      ...
      if (!appFormId) {
        setErrorResponse(
          "An application form is required to save the review form"
        );
      } else {
  ...
  const EventMeta = ({ dateFormat, saved, evnt }) => {
    return (
      <div className="dates-container">
        <span className="dates">
          {t("Review opens ") + " :"}
          <span className="date">{`${dateFormat(evnt.review_open)}`}</span>
        </span>
        <span className="dates">
          {t("Review closes ") + " :"}
          <span className="date">{`${dateFormat(evnt.review_close)}`}</span>
        </span>
        {!stage.loading && (
          <Stages
            t={t}
