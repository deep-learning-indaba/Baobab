...
class ReferenceComponent extends Component {
  ...
  getDetails = () => {
    referenceService
      .getReferenceRequestDetails(this.props.match.params.token)
      .then((response) => {
        this.setState({
          ...
          submitted:
            response.details &&
            !!response.details.reference_submitted_timestamp,
        });
      });
  };
  ...
  render() {
    ...
    if (!requestDetails.is_application_open) {
      ...
      return (
        <div className={"alert alert-danger alert-container"}>
          <Trans i18nKey="referencesClosed">
            Unfortunately applications for {{ eventName }} are now closed. If
            you still need to submit a reference for {{ candidateName }}, please{" "}
            <a href={`mailto:${requestDetails.email_from}`}>
      ...
      <div>
        ...
        {submittedTimestamp && (
          <div className="alert alert-success alert-container">
            <Trans>
              Thank you! You submitted your reference on{" "}
              {{ submittedTimestamp }}. You may update it below if you like.
            </Trans>
          </div>
        )}

        <p>
          {candidateName} {nominationText} {t("for")}{" "}
          {requestDetails.description}.{" "}
          {t(
            "Kindly upload your reference in support of their application in the form of a PDF (max 10Mb) below."
          )}
        </p>
