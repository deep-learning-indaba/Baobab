...
// Please integrate Translation functionality before testing can occur

/*
import React from "react"
import { shallow, mount } from 'enzyme';
import FormMultiFile, { FormMultiFile as OriginalFormMultiFile } from '../FormMultiFile.js';
...
describe('FormMultiFile Tests', () => {
    it('Check if FormMultiFile Page renders.', () => {
        const wrapper = shallow(<OriginalFormMultiFile />);
    ...
    it('Check if FormMultiFile state updates in rendered().', () => {
        ...
        expect(rapper.find('.multi-file-component').length).toEqual(2);
    })


    it('Check if FormMultiFile adds upload components render().', () => {
        const wrapper = shallow(<OriginalFormMultiFile />);
        wrapper.setState({ fileList: [{ name: "test", file: "test", delete: false, filePath: "1234" }] });
