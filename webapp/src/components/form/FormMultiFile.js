  ...
  handleUpload = (id, file, name, rename) => {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (!existing.file) {
        ...
        // Already uploaded, update metadata
        existing.name = name;
        this.setState({ fileList: handleList }, () => {
          if (this.props.onChange) {
            const filteredList = this.state.fileList.filter((f) => f.file);
      ...
      <div>
        <FormGroup id={this.props.id + "-group"}>
          ...
          {this.state.fileList.map((val) => {
            return (
              <MultiFileComponent
                className="multi-file-component"
          ...
          {canAddFile && (
            <button className="add-file-btn" onClick={(e) => this.addFile(e)}>
              {t("Add File")}
            </button>
          )}
          <div
            className={this.props.errorText ? "errorText display" : "errorText"}
          >
            {" "}
