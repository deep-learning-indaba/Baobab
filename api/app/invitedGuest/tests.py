...
INVITED_GUEST = {"event_id": 1, "email": "something@email.com", "role": "jedi"}

INVITED_GUEST_2 = {"event_id": 1, "email": "something2@email.com", "role": "jedi"}

INVITED_GUEST_NEW_USER = {"event_id": 1, "email": "new@email.com", "role": "jedi"}
USER_DATA = {
...
class InvitedGuestTest(ApiTestCase):
    ...
    def test_create_invitedGuest_duplicate(self):
        self.seed_static_data()
        self.app.post("/api/v1/invitedGuest", data=INVITED_GUEST, headers=self.headers)
        response = self.app.post(
            "/api/v1/invitedGuest", data=INVITED_GUEST, headers=self.headers
        )
    ...
    def test_create_invitedGuest_create_user(self):
        ...
        data = json.loads(response.data)
        assert response.status_code == 201
        assert data["fullname"] == "{} {} {}".format(
            USER_DATA["user_title"], USER_DATA["firstname"], USER_DATA["lastname"]
        )
    ...
    def test_create_invitedGuest_list(self):
        ...
        response = self.app.get(
            "/api/v1/invitedGuestList?event_id=" + str(event_id),
            headers=self.adminHeaders,
        )
        data = json.loads(response.data)
        data = sorted(data, key=lambda k: k["invited_guest_id"])
        self.assertEqual(len(data), 2)
        self.assertEqual(data[0]["user"]["email"], INVITED_GUEST["email"])
        self.assertEqual(data[1]["user"]["email"], INVITED_GUEST_2["email"])
