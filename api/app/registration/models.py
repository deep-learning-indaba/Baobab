...
def get_registration_answer_based_headline(user_id, headline):
    question = (
        ...
        .filter(func.lower(RegistrationQuestion.headline) == func.lower(headline))
        .first()
    )
    answer = None
...
class RegistrationAnswer(db.Model):
    ...
    registration_question_id = db.Column(
        db.Integer(), db.ForeignKey("registration_question.id"), nullable=False
    )
