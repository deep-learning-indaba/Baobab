...
from app.registration.mixins import (
    OfferMixin,
    RegistrationFormMixin,
    RegistrationQuestionMixin,
    RegistrationSectionMixin,
)
...
from app.utils.auth import admin_required, auth_required, verify_token
...
def offer_info(offer_entity, requested_travel=None):
    return {
        ...
        and offer_entity.responded_at.strftime("%Y-%m-%d"),
        ...
        "requested_travel": requested_travel
        and (
            requested_travel.value == "transport"
            or requested_travel.value == "transport-accommodation"
        ),
        "requested_accommodation": requested_travel
        and (
            requested_travel.value == "accommodation"
            or requested_travel.value == "transport-accommodation"
        ),
...
class OfferAPI(OfferMixin, restful.Resource):
    ...
    def put(self):
        ...
        LOGGER.info(
            "Updating offer {} with values: candidate response: {}, Accepted accommodation: {}, "
            "Accepted travel: {}, Rejected Reason: {}".format(
                offer_id,
                candidate_response,
        ...
        try:
            ...
            LOGGER.error(
                "Failed to update offer with id {} due to {}".format(
                    args["offer_id"], e
                )
            )
    ...
    def post(self):
        ...
        existing_offer = (
            ...
            .filter(Offer.user_id == user_id, Offer.event_id == event_id)
            .first()
        )
        if existing_offer:
        ...
        new_outcome = Outcome(event_id, user_id, Status.ACCEPTED, g.current_user["id"])
    ...
    def get(self):
        ...
        try:
            ...
            response = response_repository.get_submitted_by_user_id_for_event(
                user_id, event_id
            )
            if not response:
                return errors.RESPONSE_NOT_FOUND
            request_travel = (
                response_repository.get_answer_by_question_key_and_response_id(
                    "travel_grant", response.id
                )
            )
...
class RegistrationFormAPI(RegistrationFormMixin, restful.Resource):
    ...
    registration_section_fields = {
        ...
        "description": fields.String,
        "order": fields.Integer,
        "registration_questions": fields.List(
            fields.Nested(registration_question_fields)
        ),
    }
    ...
    registration_form_fields = {
        "id": fields.Integer,
        "event_id": fields.Integer,
        "registration_sections": fields.List(
            fields.Nested(registration_section_fields)
        ),
    }
    ...
    def get(self):
        ...
        try:
            ...
            sections = (
                db.session.query(RegistrationSection)
                .filter(
                    RegistrationSection.registration_form_id == registration_form.id
                )
                .all()
            )
            ...
            for section in sections:
                ...
                if (
                    (section.show_for_travel_award is None)
                    and (section.show_for_accommodation_award is None)
                    and (section.show_for_payment_required is None)
                ):
                    included_sections.append(section)

                elif (
                    (
                        section.show_for_travel_award
                        and offer.travel_award
                        and offer.accepted_travel_award
                    )
                    or (
                        section.show_for_accommodation_award
                        and offer.accommodation_award
                        and offer.accepted_accommodation_award
                    )
                    or (section.show_for_payment_required and offer.payment_required)
                ):
                    included_sections.append(section)
            ...
            questions = (
                ...
                .filter(
                    RegistrationQuestion.registration_form_id == registration_form.id
                )
                .all()
            )
    ...
    def post(self):
        ...
        registration_form = RegistrationForm(event_id=event_id)
...
class RegistrationSectionAPI(RegistrationSectionMixin, restful.Resource):
    ...
    def get(self):
        ...
        try:
            ...
            registration_section = (
                db.session.query(RegistrationSection)
                .filter(RegistrationSection.id == section_id)
                .first()
            )
    ...
    def post(self):
        ...
        registration_form = (
            db.session.query(RegistrationForm)
            .filter(RegistrationForm.id == registration_form_id)
            .first()
        )
        ...
        try:
            ...
            LOGGER.error(
                "Failed to add registration section with form id : {}".format(
...
class RegistrationQuestionAPI(RegistrationQuestionMixin, restful.Resource):
    ...
    def get(self):
        ...
        try:
            ...
            question = (
                db.session.query(RegistrationQuestion)
                .filter(RegistrationQuestion.id == question_id)
                .first()
            )
    ...
    def post(self):
        ...
        try:
            ...
            LOGGER.error(
                "Failed to add registration question with form id : {}".format(
                    section_id
                )
            )
