...
class AttendanceApiTest(ApiTestCase):
    def seed_static_data(self):
        ...
        registration = Registration(
            offer_id=offer.id, registration_form_id=form.id, confirmed=True
        )

        db.session.add_all([registration])
        db.session.commit()
        ra = RegistrationAnswer(
            registration_id=registration.id, registration_question_id=rq.id, value="yes"
        )
    ...
    def test_get_attendance_list(self):
        ...
        registration2 = Registration(
            offer_id=offer2.id, registration_form_id=self.form.id, confirmed=False
        )
    ...
    def test_get_attendance_list_2(self):
        ...
        role = "EveryRole"
        mrObamaInvitedGuest = InvitedGuest(
            event_id=1, user_id=invited_guest_id, role=role
        )
        ...
        attendance_response = self.app.post(
            "/api/v1/attendance", headers=header, data=params
        )
    ...
    def test_get_attendance_list_3(self):
        ...
        dupl_attendee = InvitedGuest(
            event_id=1, user_id=invited_guest_id, role="EveryRole"
        )
        ...
        data = json.loads(result.data)
        occurences_in_attendance_list = [
            att for att in data if att["user_id"] == invited_guest_id
        ]
