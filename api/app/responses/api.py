...
from app.utils import emailer, errors, pdfconvertor, storage, strings, zipping
...
class ResponseAPI(ResponseMixin, restful.Resource):
    ...
    def post(self):
        ...
        for answer_args in args["answers"]:
            answer = Answer(
                response.id, answer_args["question_id"], answer_args["value"]
            )
        ...
        try:
            if response.is_submitted:
                LOGGER.info(
                    "Sending confirmation email for response with ID : {id}".format(
                        id=response.id
                    )
                )
                user = user_repository.get_by_id(user_id)
            ...
            LOGGER.warn(
                "Failed to send confirmation email for response with ID : {id}, but the response was submitted succesfully".format(
                    id=response.id
                )
            )
        finally:
    ...
    def delete(self):
        ...
        try:
            ...
            LOGGER.error(
                "Failed to send withdrawal confirmation email for response with ID : {id}, but the response was withdrawn succesfully".format(
                    id=args["id"]
                )
            )
    ...
    def send_confirmation(self, user, response):
        try:
            ...
            if not answers:
                LOGGER.warn(
                    "Found no answers associated with response with id {response_id}".format(
            ...
            if event is None:
                LOGGER.warn(
                    "Found no event id {event_id}".format(
                        form_id=application_form.event_id
                    )
                )
        except:
            LOGGER.error(
                "Could not connect to the database to retrieve response confirmation email data on response with ID : {response_id}".format(
                    response_id=response.id
                )
            )
        ...
        try:
            ...
            LOGGER.error(
                "Could not send confirmation email for response with id : {response_id} due to: {e}".format(
                    response_id=response.id, e=e
                )
            )
...
def _serialize_reviewer(response_reviewer, review_response):
    ...
    return {
        ...
        "reviewer_name": "{} {} {}".format(
            response_reviewer.user.user_title,
            response_reviewer.user.firstname,
            response_reviewer.user.lastname,
        ),
...
def _serialize_answer(answer, language):
    ...
    if translation is None:
        LOGGER.warn(
            "No {} translation found for question id {}".format(language, question.id)
        )
...
def _serialize_tag(tag, language):
    ...
    if not tag_translation:
        LOGGER.warn(
            "Could not find {} translation for tag id {}".format(language, tag.id)
        )
...
class ResponseListAPI(restful.Resource):
    ...
    def get(self, event_id):
        ...
        req_parser.add_argument(
            "include_unsubmitted", type=inputs.boolean, required=True
        )
        # Note: Including [] in the question_ids parameter because that gets added by Axios on the front-end
        req_parser.add_argument(
            "question_ids[]", type=int, required=False, action="append"
        )
        ...
        responses = response_repository.get_all_for_event(
            event_id, not include_unsubmitted
        )

        review_config = review_configuration_repository.get_configuration_for_event(
            event_id
        )
        required_reviewers = (
            1
            if review_config is None
            else review_config.num_reviews_required + review_config.num_optional_reviews
        )

        response_reviewers = review_repository.get_response_reviewers_for_event(
            event_id
        )
        response_to_reviewers = {
            k: list(g)
            for k, g in itertools.groupby(response_reviewers, lambda r: r.response_id)
        }

        review_responses = review_repository.get_review_responses_for_event(event_id)
        reviewer_to_review_response = {r.reviewer_user_id: r for r in review_responses}

        serialized_responses = []
        for response in responses:
            reviewers = [
                _serialize_reviewer(
                    r, reviewer_to_review_response.get(r.reviewer_user_id, None)
                )
            ...
            if question_ids:
                answers = [
                    _serialize_answer(answer, language)
            ...
            serialized = {
                ...
                "submitted_date": None
                if response.submitted_timestamp is None
                else response.submitted_timestamp.isoformat(),
                "language": response.language,
                "answers": answers,
                "reviewers": reviewers,
                "tags": [
                    _serialize_tag(rt.tag, language) for rt in response.response_tags
                ],
            }
...
class ResponseTagAPI(restful.Resource, ResponseTagMixin):
    ...
    def post(self):
        ...
        if not _validate_user_admin_or_reviewer(
            g.current_user["id"], event_id, response_id
        ):
...
class ResponseDetailAPI(restful.Resource):
    ...
    def _serialize_reviewer(response_reviewer, review_form_id):
        if review_form_id is None:
            ...
            review_response = review_repository.get_review_response(
                review_form_id,
                response_reviewer.response_id,
                response_reviewer.reviewer_user_id,
            )
    ...
    def _serialize_response(response, language, review_form_id, num_reviewers):
        return {
            ...
            "is_submitted": response.is_submitted,
            "submitted_timestamp": ResponseDetailAPI._serialize_date(
                response.submitted_timestamp
            ),
            "is_withdrawn": response.is_withdrawn,
            "withdrawn_timestamp": ResponseDetailAPI._serialize_date(
                response.withdrawn_timestamp
            ),
            "started_timestamp": ResponseDetailAPI._serialize_date(
                response.started_timestamp
            ),
            "answers": [
                ResponseDetailAPI._serialize_answer(answer)
                for answer in response.answers
            ],
            "language": response.language,
            "user_title": response.user.user_title,
            ...
            "lastname": response.user.lastname,
            "tags": [
                ResponseDetailAPI._serialize_tag(rt.tag, language)
                for rt in response.response_tags
            ],
            "reviewers": [
                ResponseDetailAPI._serialize_reviewer(r, review_form_id)
                for r in response.reviewers
            ],
        }
    ...
    def get(self, event_id):
        ...
        num_reviewers = (
            review_config.num_reviews_required + review_config.num_optional_reviews
            if review_config is not None
            else 1
        )

        return ResponseDetailAPI._serialize_response(
            response, language, review_form_id, num_reviewers
        )
...
class ResponseExportAPI(restful.Resource):
    def get(self):
        ...
        response_string = strings.build_response_html_app_info(
            response, language
        ) + strings.build_response_html_answers(
            response.answers, language, application_form
        )

        # Convert to PDF
        files_to_compress = [
            ("response.pdf", pdfconvertor.html_to_pdf(response.id, response_string))
        ]
