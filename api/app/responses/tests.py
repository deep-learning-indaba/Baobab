...
class ResponseApiTest(ApiTestCase):
    ...
    def _seed_data(self):
        ...
        self.event = self.add_event(
            {"en": "Event Without Nomination"}, key="indaba-2025"
        )
        ...
        self.question_translation = self.add_question_translation(
            self.question.id, "en", "Question 1"
        )
        ...
        self.question_translation2 = self.add_question_translation(
            self.question2.id, "en", "Question 2"
        )
        self.response = self.add_response(
            self.form.id, self.other_user_data["id"], False, False
        )
        self.answer1 = self.add_answer(self.response.id, self.question.id, "My Answer")

        self.event_with_nomination = self.add_event(
            {"en": "Event With Nomination"}, key="eeml-2025"
        )
        self.form_with_nomination = self.create_application_form(
            self.event_with_nomination.id, True, True
        )
        self.section_with_nomination = self.add_section(self.event_with_nomination.id)
        self.section_translation_with_nomination = self.add_section_translation(
            self.section_with_nomination.id, "en"
        )
        self.question1_with_nomination = self.add_question(
            self.form_with_nomination.id, self.section_with_nomination.id, order=1
        )
        self.question_translation1_with_nomination = self.add_question_translation(
            self.question1_with_nomination.id, "en", "Question 1 with nomination"
        )
        self.question2_with_nomination = self.add_question(
            self.form_with_nomination.id, self.section_with_nomination.id, order=2
        )
        self.question_translation2_with_nomination = self.add_question_translation(
            self.question2_with_nomination.id, "en", "Question 1 with nomination"
        )
        self.response1_with_nomination = self.add_response(
            self.form_with_nomination.id, self.other_user_data["id"], True, False
        )
        self.answer2_with_nomination = self.add_answer(
            self.response1_with_nomination.id,
            self.question2_with_nomination.id,
            "Second nomination answer",
        )
        self.answer1_with_nomination = self.add_answer(
            self.response1_with_nomination.id,
            self.question1_with_nomination.id,
            "Another answer",
        )
        self.response2_with_nomination = self.add_response(
            self.form_with_nomination.id, self.other_user_data["id"], False, True
        )
        self.answer4_with_nomination = self.add_answer(
            self.response2_with_nomination.id,
            self.question2_with_nomination.id,
            "Second answer for second nomination",
        )
        self.answer3_with_nomination = self.add_answer(
            self.response2_with_nomination.id,
            self.question1_with_nomination.id,
            "First answer for second nomination",
        )
    ...
    def test_get_response_with_nomination(self):
        ...
        self.assertEqual(
            answer1["question_id"], self.answer1_with_nomination.question_id
        )
        ...
        self.assertEqual(
            answer2["question_id"], self.answer2_with_nomination.question_id
        )
        ...
        self.assertEqual(
            answer3["question_id"], self.answer3_with_nomination.question_id
        )
        ...
        self.assertEqual(
            answer4["question_id"], self.answer4_with_nomination.question_id
        )
    ...
    def test_post_without_nomination(self):
        ...
        response_data = {
            ...
            "is_submitted": True,
            "answers": [
                {"question_id": self.question.id, "value": "Answer 1"},
                {
    ...
    def test_second_response_rejected_without_nomination(self):
        ...
        response_data = {
            "application_form_id": self.form.id,
            "is_submitted": True,
            "answers": [{"question_id": self.question.id, "value": "Answer 1"}],
        }
    ...
    def test_second_response_accepted_with_nomination(self):
        ...
        response_data = {
            ...
            "is_submitted": True,
            "answers": [
                {"question_id": self.question1_with_nomination.id, "value": "Answer 1"}
            ],
        }
    ...
    def test_update(self):
        ...
        response = self.app.put(
            ...
            content_type="application/json",
            headers={"Authorization": self.other_user_data["token"]},
            query_string={"language": "fr"},
        )  # Updating the language from English to French
...
class ResponseListAPITest(ApiTestCase):
    def _seed_static_data(self):
        ...
        UserName = collections.namedtuple(
            "UserName", ["user_title", "firstname", "lastname"]
        )
        self.user_names = [
            UserName(u.user_title, u.firstname, u.lastname) for u in self.users
        ]
        ...
        self.form1_question1 = self.add_question(
            self.form1.id, self.form1_section1.id
        )  # ID 1
        self.add_question_translation(
            self.form1_question1.id, "en", "English Headline 1"
        )
        self.add_question_translation(
            self.form1_question1.id, "fr", "French Headline 1"
        )
        self.form1_question2 = self.add_question(
            self.form1.id, self.form1_section1.id
        )  # ID 2
        self.add_question_translation(
            self.form1_question2.id, "en", "English Headline 2"
        )
        self.add_question_translation(
            self.form1_question2.id, "fr", "French Headline 2"
        )
        self.form1_question3 = self.add_question(
            self.form1.id, self.form1_section1.id
        )  # ID 3
        self.add_question_translation(
            self.form1_question3.id, "en", "English Headline 3"
        )
        self.add_question_translation(
            self.form1_question3.id, "fr", "French Headline 3"
        )

        self.form2 = self.create_application_form(self.event2.id)
        self.form2_section1 = self.add_section(self.form2.id)
        self.form2_question1 = self.add_question(
            self.form2.id, self.form2_section1.id
        )  # ID 4
        self.add_question_translation(
            self.form2_question1.id, "en", "English Headline 3"
        )
        self.add_question_translation(
            self.form2_question1.id, "fr", "French Headline 3"
        )

        # Create responses, 3 for event 1 (2 submitted, 1 unsubmitted), 1 for event 2
        self.response1 = self.add_response(
            self.form1.id, self.users[0].id, is_submitted=True
        )
        ...
        self.response2 = self.add_response(
            self.form1.id, self.users[1].id, is_submitted=True, language="fr"
        )
        ...
        self.response3 = self.add_response(
            self.form1.id, self.users[2].id, is_submitted=False
        )
        ...
        self.response4 = self.add_response(
            self.form2.id, self.users[3].id, is_submitted=False
        )
        ...
        review_response = self.add_review_response(
            self.users[1].id, self.response1.id, self.review_form1.id
        )
    ...
    def test_no_questions_submitted(self):
        ...
        self.assertEqual(
            response1["submitted_date"], self.response1_submitted.isoformat()
        )
        ...
        self.assertEqual(
            response1["reviewers"][0]["reviewer_name"],
            "{} {} {}".format(*self.user_names[1]),
        )
        self.assertEqual(
            response1["reviewers"][0]["review_response_id"], self.review_response_id
        )
        self.assertEqual(response1["reviewers"][1]["reviewer_id"], 3)
        self.assertEqual(
            response1["reviewers"][1]["reviewer_name"],
            "{} {} {}".format(*self.user_names[2]),
        )
        ...
        self.assertEqual(
            response2["submitted_date"], self.response2_submitted.isoformat()
        )
        ...
        self.assertEqual(
            response2["reviewers"][0]["reviewer_name"],
            "{} {} {}".format(*self.user_names[0]),
        )
...
class ResponseDetailAPITest(ApiTestCase):
    def _seed_static_data(self):
        ...
        self.user1 = self.add_user(
            "user1@mail.com", user_title="Ms", firstname="Danai", lastname="Gurira"
        )

        self.reviewer1 = self.add_user(
            "reviewer1@mail.com", user_title="Mx", firstname="Skittles", lastname="Cat"
        )
        self.reviewer2 = self.add_user(
            "reviewer2@mail.com", user_title="Mr", firstname="Finn", lastname="Dog"
        )
        self.reviewer3 = self.add_user(
            "reviewer3@mail.com", user_title="Mx", firstname="Bailey", lastname="Dog"
        )
        ...
        self.add_review_response(
            self.reviewer2.id, self.response1.id, review_form.id, is_submitted=True
        )
    ...
    def test_response_detail(self):
        ...
        self.assertEqual(
            data["submitted_timestamp"], self.response1_submitted.isoformat()
        )
...
class ResponseExportAPITest(ApiTestCase):
    def _data_seed_static(self):
        ...
        question_supp1 = self.add_question(
            application_form.id, section1.id, question_type="file"
        )
        ...
        question_supp2 = self.add_question(
            application_form.id, section1.id, question_type="file"
        )
        self.add_question_translation(
            question_supp2.id, "en", headline="Upload references"
        )
    ...
    def test_zipped_file_uncorrupted(self):
        ...
        params = {"response_id": self.response1_id, "language": "en"}
        ...
        self.assertEqual(
            ...
            "attachment; filename=response_1.zip",
        )
    ...
    def test_filename_renamed_correctly_in_zip_folder(self):
        ...
        with tempfile.NamedTemporaryFile(mode="wb") as temp_zip:
            ...
            with zipfile.ZipFile(temp_zip.name) as zip:
                self.assertIsNone(zip.testzip())
                self.assertEqual(
                    zip.namelist(), ["response.pdf", "supplementarypdfONE.pdf"]
                )
