...
class ReferenceAPITest(ApiTestCase):
    ...
    def test_create_reference_request(self):
        ...
        response = self.app.post(
            '/api/v1/reference-request', data=REFERENCE_REQUEST_DETAIL, headers=self.first_headers)
    ...
    def test_get_reference_request_detail_by_token_self_nomination(self):
        ...
        reference_request_repository.create(reference_req)
        response = self.app.get(
                '/api/v1/reference-request/detail', data={'token': reference_req.token}, headers=self.first_headers)
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
        self.assertEqual(data['candidate'], 'Mx First User')
        self.assertEqual(data['relation'], reference_req.relation)
        self.assertEqual(data['name'], self.test_event_data['name']['en'])
        self.assertEqual(data['description'], self.test_event_data['description']['en'])
    ...
    def test_get_reference_request_detail_by_token_other_nomination(self):
        self._seed_data()
        reference_req = ReferenceRequest(2, 'Mr', 'John', 'Snow', 'Supervisor', 'common@email.com')
        reference_request_repository.create(reference_req)
        response = self.app.get(
                '/api/v1/reference-request/detail', data={'token': reference_req.token}, headers=self.other_headers)
        data = json.loads(response.data)
        self.assertEqual(response.status_code, 200)
