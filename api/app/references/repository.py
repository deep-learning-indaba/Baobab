...
class ReferenceRequestRepository:
    ...
    def get_all_by_response_id(response_id):
        return (
            db.session.query(ReferenceRequest).filter_by(response_id=response_id).all()
        )

    @staticmethod
    def get_by_token(token):
        return db.session.query(ReferenceRequest).filter_by(token=token).first()

    @staticmethod
    def get_references_by_response_id(response_id):
        return (
            db.session.query(ReferenceRequest.response_id, Reference)
            .join(
                ReferenceRequest, ReferenceRequest.id == Reference.reference_request_id
            )
