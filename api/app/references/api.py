...
from app.references.mixins import (
    ReferenceMixin,
    ReferenceRequestDetailMixin,
    ReferenceRequestsListMixin,
    ReferenceRequestsMixin,
)
...
def _get_candidate_nominator(response):
    ...
    if is_nomination:
        question_answers = (
            response_repository.get_question_answers_by_section_key_and_response_id(
                "nominee_section", response.id
            )
        )

        nomination_info = {qa.Question.key: qa.Answer.value for qa in question_answers}
        candidate = (
            "{nomination_title} {nomination_firstname} {nomination_lastname}".format(
                **nomination_info
            )
        )
...
class ReferenceRequestAPI(ReferenceRequestsMixin, restful.Resource):
    ...
    def post(self):
        ...
        link = "{host}/reference/{token}".format(
            host=misc.get_baobab_host(), token=reference_request.token
        )
...
class ReferenceRequestDetailAPI(ReferenceRequestDetailMixin, restful.Resource):
    ...
    def get(self):
        ...
        return_object = {
            ...
            "reference_submitted_timestamp": reference.timestamp
...
class ReferenceAPI(ReferenceMixin, restful.Resource):
    ...
    def get(self):
        ...
        if not user.is_event_admin(event.id):
            return FORBIDDEN
        reference_responses = (
            reference_request_repository.get_references_by_response_id(response.id)
        )
        return [
            reference_response.Reference for reference_response in reference_responses
        ], 200
