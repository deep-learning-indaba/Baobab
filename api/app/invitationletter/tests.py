...
class InvitationLetterTests(ApiTestCase):
    def seed_static_data(self):
        ...
        registration_2 = Registration(
            offer_id=offer_2.id, registration_form_id=form.id, confirmed=True
        )
    ...
    def test_create_create_invitation_letter(self):
        ...
        letter = (
            db.session.query(InvitationLetterRequest)
            .filter(InvitationLetterRequest.id == data["invitation_letter_request_id"])
            .first()
        )
    ...
    def test_generator(self):
        ...
        self.assertEqual(
            generate(
                ...
                lastname="Jeffdejeff",
                bringing_poster="",
                expiry_date=datetime(1984, 12, 12).strftime("%Y-%m-%d"),
            ),
            True,
            self.test_user,
        )
