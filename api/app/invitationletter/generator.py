...
from app.utils import emailer, errors, pdfconvertor
from config import (
    FILE_SIZE_LIMIT,
    GCP_BUCKET_NAME,
...
def check_values(
    ...
    assert residential_address is not None and isinstance(
        residential_address, string_types
    )
    ...
    assert passport_issued_by is not None and isinstance(
        passport_issued_by, string_types
    )
    assert invitation_letter_sent_at is not None and isinstance(
        invitation_letter_sent_at, string_types
    )
    ...
    assert country_of_residence is not None and isinstance(
        country_of_residence, string_types
    )
...
def generate(
    ...
    success = pdfconvertor.convert_to(
        folder="app/invitationletter/letter",
