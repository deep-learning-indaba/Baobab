...
class ReviewRepository:
    ...
    def get_review_form(event_id, stage=None):
        ...
        review_form = (
            query.join(
                ApplicationForm, ApplicationForm.id == ReviewForm.application_form_id
            )
        ...
        remaining = (
            ...
            .outerjoin(
                ...
                and_(
                    ...
                    ReviewResponse.reviewer_user_id == reviewer_user_id,
                ),
            )
            .filter(
                or_(ReviewResponse.id == None, ReviewResponse.is_submitted == False)
            )
            .all()[0][0]
        )
        return remaining
    ...
    def get_review_history(reviewer_user_id, event_id):
        reviews = (
            db.session.query(
                ReviewResponse.id, ReviewResponse.submitted_timestamp, AppUser, Response
            )
            .filter(ReviewResponse.reviewer_user_id == reviewer_user_id)
            .join(ReviewForm, ReviewForm.id == ReviewResponse.review_form_id)
            .filter(
                ReviewForm.application_form_id == event_id, ReviewForm.active == True
            )
    ...
    def get_review_list(reviewer_user_id, event_id):
        reviews = (
            db.session.query(Response, ReviewResponse)
            .join(ResponseReviewer, Response.id == ResponseReviewer.response_id)
            .filter_by(reviewer_user_id=reviewer_user_id)
            .join(ApplicationForm, Response.application_form_id == ApplicationForm.id)
            .filter_by(event_id=event_id)
            .outerjoin(
                ReviewResponse,
                and_(
                    Response.id == ReviewResponse.response_id,
                    ReviewResponse.reviewer_user_id == reviewer_user_id,
                ),
            )
        )
        ...
        count = (
            ...
            .outerjoin(
                ...
                and_(
                    ...
                    ReviewResponse.reviewer_user_id
                    == ResponseReviewer.reviewer_user_id,
                ),
            )
    ...
    def get_review_complete_timeseries_by_event(event_id):
        timeseries = (
            db.session.query(
                cast(ReviewResponse.submitted_timestamp, Date),
                func.count(ReviewResponse.submitted_timestamp),
            )
            .join(ReviewForm, ReviewForm.id == ReviewResponse.review_form_id)
            .filter(ReviewForm.active == True)
            .join(ApplicationForm, ReviewForm.application_form_id == ApplicationForm.id)
    ...
    def get_already_assigned(reviewer_user_id):
        already_assigned = (
            db.session.query(ResponseReviewer.response_id)
    ...
    def get_form_id(event_id):
        form_id = (
            db.session.query(ApplicationForm.id).filter_by(event_id=event_id).first()[0]
        )
        return form_id
    ...
    def deactivate_review(review_response):
        scores = (
            ...
            .filter(
                ReviewScore.review_response_id == review_response.id,
    ...
    def get_candidate_response_ids(event_id, reviewer_user_id, reviews_required):
        candidate_response_ids = (
            ...
            .filter(
                Response.user_id != reviewer_user_id,
    ...
    def delete_response_reviewer(response_id, reviewer_user_id):
        db.session.query(ResponseReviewer).filter_by(
            response_id=response_id, reviewer_user_id=reviewer_user_id
        ).delete()
        db.session.commit()
    ...
    @staticmethod
    def get_average_score_for_review_question(
        response_id: int, review_question_id: int
    ):
        review_score_values = (
            db.session.query(ReviewScore.value)
            .filter(
                ReviewScore.review_question_id == review_question_id,
                ReviewScore.is_active == True,
            )
        ...
        review_score_values = [
            misc.try_parse_float(review_score_value[0])
            for review_score_value in review_score_values
        ]
    ...
    def delete_review_question(question_to_delete: ReviewQuestion):
        db.session.query(ReviewQuestionTranslation).filter_by(
            review_question_id=question_to_delete.id
        ).delete()
    ...
    def delete_review_section(section: ReviewSection):
        db.session.query(ReviewSectionTranslation).filter_by(
...
class ReviewConfigurationRepository:
    ...
    def get_configuration_for_form(review_form_id):
        config = (
            db.session.query(ReviewConfiguration)
