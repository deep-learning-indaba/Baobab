...
from app.references.models import Reference, ReferenceRequest
...
from app.utils.errors import FORBIDDEN, REVIEW_RESPONSE_NOT_FOUND, USER_NOT_FOUND
...
class ReviewsApiTest(ApiTestCase):
    def seed_static_data(self):
        ...
        event_admin = AppUser(
            "ea@ea.com", "event_admin", "1", "Ms", password="abc", organisation_id=1
        )
        users = [
            reviewer1,
            reviewer2,
        ...
        events = [
            self.add_event(
                {"en": "indaba 2019"},
                {
                    "en": "The Deep Learning Indaba 2019, Kenyatta University, Nairobi, Kenya "
                },
                datetime(2019, 8, 25),
                datetime(2019, 8, 31),
                "KENYADABA2019",
            ...
            self.add_event(
                {"en": "indaba 2020"},
                {
                    "en": "The Deep Learning Indaba 2018, Stellenbosch University, South Africa"
                },
                datetime(2018, 9, 9),
                datetime(2018, 9, 15),
                "INDABA2020",
        ...
        event_roles = [EventRole("admin", 10, 1), EventRole("reviewer", 3, 1)]
        ...
        sections = [Section(1, 1), Section(2, 1)]
        ...
        self.add_question_translation(
            5, "en", "Did you attend the 2017 or 2018 Indaba", options=options
        )
        ...
        review_configs = [
            ReviewConfiguration(
                review_form_id=review_forms[0].id,
                num_reviews_required=3,
                num_optional_reviews=0,
            ),
            ReviewConfiguration(
                review_form_id=review_forms[1].id,
                num_reviews_required=3,
                num_optional_reviews=0,
            ),
        ]
        ...
        self.add_review_section_translation(
            review_section1.id,
            "en",
            "Review Section 1 English",
            "Review Section 1 Description English",
        )
        self.add_review_section_translation(
            review_section1.id,
            "fr",
            "Review Section 1 French",
            "Review Section 1 Description French",
        )
        review_section2 = self.add_review_section(review_forms[1].id)
        self.add_review_section_translation(
            review_section2.id, "en", "Review Section 2", "Review Section 2 Description"
        )
        ...
        review_question_translations = [
            ...
            ReviewQuestionTranslation(
                review_questions[1].id,
                "en",
                headline="English Headline",
                description="English Description",
                placeholder="English Placeholder",
                options=[{"label": "en1", "value": "en"}],
                validation_regex="EN Regex",
                validation_text="EN Validation Message",
            ),
            ReviewQuestionTranslation(
                review_questions[1].id,
                "fr",
                ...
                options=[{"label": "fr1", "value": "fr"}],
                validation_regex="FR Regex",
                validation_text="FR Validation Message",
            ),
    ...
    def test_one_reviewer_one_candidate_review_summary(self, active):
        ...
        response = self.app.get(
            "/api/v1/reviewassignment/summary", headers=header, data=params
        )
        data = json.loads(response.data)
    ...
    def setup_one_reviewer_three_candidates_with_one_withdrawn_response_and_one_unsubmitted_response(
        self,
    ):
    ...
    def test_one_reviewer_three_candidates_with_one_withdrawn_response_and_one_unsubmitted_response(
        self,
    ):
        self.seed_static_data()
    ...
    def setup_multiple_reviewers_with_different_subsets_of_candidates_and_reviews_completed(
        self,
    ):
        self.add_response(1, 5, is_submitted=True)
        self.add_response(1, 6, is_submitted=True)
    ...
    def test_multiple_reviewers_with_different_subsets_of_candidates_and_reviews_completed(
        self,
    ):
        self.seed_static_data()
    ...
    def test_skipping(self):
        ...
        self.assertEqual(
            data["response"]["answers"][0]["value"], "I want to solve new problems."
        )
    ...
    def test_high_skip_defaults_to_last_review(self):
        ...
        self.assertEqual(data["response"]["user_id"], 7)
        self.assertEqual(
            data["response"]["answers"][1]["value"], "I will share by tutoring."
        )
    ...
    def setup_candidate_who_has_applied_to_multiple_events(self):
        ...
        response_reviewers = [ResponseReviewer(1, 1), ResponseReviewer(2, 1)]
        db.session.add_all(response_reviewers)
        db.session.commit()
    ...
    def test_filtering_on_event_when_candidate_has_applied_to_more_than(self):
        ...
        self.assertEqual(
            data["response"]["answers"][0]["value"], "Yes I worked on a vision task."
        )
    ...
    def test_multi_choice_answers_use_label_instead_of_value(self):
        ...
        self.assertEqual(
            data["response"]["answers"][0]["value"], "Yes, I attended the 2017 Indaba"
        )
        ...
        params = json.dumps(
            {
                "review_form_id": 1,
                "response_id": 1,
                "scores": [{"review_question_id": 1, "value": "test_answer"}],
                "language": "en",
                "is_submitted": False,
            }
        )
    ...
    def test_saving_review_response(self):
        ...
        review_scores = (
            db.session.query(ReviewScore)
            .filter_by(review_response_id=1, is_active=True)
            .all()
        )
        self.assertEqual(response.status_code, 201)
    ...
    def setup_existing_review_response(self):
        ...
        review_response = ReviewResponse(1, 1, 1, "en")
        review_response.submit()
        review_response.review_scores = [
            ReviewScore(1, "test_answer1"),
            ReviewScore(2, "test_answer2"),
        ]
        db.session.add(review_response)
        db.session.commit()
    ...
    def test_updating_review_response(self):
        ...
        params = json.dumps(
            {
                ...
                "response_id": 1,
                "scores": [
                    {"review_question_id": 1, "value": "test_answer3"},
                    {"review_question_id": 2, "value": "test_answer4"},
                ],
                "language": "en",
                "is_submitted": True,
            }
        ...
        review_scores = (
            db.session.query(ReviewScore)
            .filter_by(review_response_id=1, is_active=True)
            .order_by(ReviewScore.review_question_id)
            .all()
        )
    ...
    def test_user_cant_assign_responsesreviewer_without_system_or_event_admin_role(
        self,
    ):
        self.seed_static_data()
        ...
        response = self.app.post(
            "/api/v1/reviewassignment", headers=header, data=params
        )
    ...
    def test_reviewer_not_found(self):
        ...
        params = {
            "event_id": 1,
            "reviewer_user_email": "non_existent@user.com",
            "num_reviews": 10,
        }
    ...
    def test_add_reviewer_with_a_role(self):
        ...
        event_roles = (
            db.session.query(EventRole)
            .filter_by(user_id=10, event_id=1)
    ...
    def test_adding_first_reviewer(self):
        ...
        response_reviewers = (
            db.session.query(ResponseReviewer).filter_by(reviewer_user_id=3).all()
        )
        self.assertEqual(response.status_code, 201)
    ...
    def test_response_will_get_multiple_reviewers_assigned(self):
        ...
        response_reviewers = (
            ...
            .order_by(ResponseReviewer.reviewer_user_id)
            .all()
        )
    ...
    def setup_reviewer_responses_finalverdict_reviewquestion_reviewresponses_and_scores(
        self,
    ):
        ...
        db.session.commit()
        verdict_question_translation = ReviewQuestionTranslation(
            verdict_question.id,
            "en",
            headline="Final Verdict",
            options=final_verdict_options,
        )
        ...
        review_responses[2].review_scores = [
            ReviewScore(1, "45"),
            ReviewScore(2, "67"),
            ReviewScore(5, "No"),
        ]
    ...
    def setup_reviewresponses_with_unordered_timestamps(self):
        ...
        review_responses[0].review_scores = [
            ReviewScore(1, "67"),
            ReviewScore(5, "Yes"),
        ]
        review_responses[1].review_scores = [
            ReviewScore(1, "23"),
            ReviewScore(5, "Yes"),
        ]
        review_responses[2].review_scores = [
    ...
    def test_order_by_submittedtimestamp(self):
        ...
        LOGGER.debug(data)
        self.assertEqual(
            data["reviews"][0]["submitted_timestamp"], "2018-01-01T00:00:00"
        )
        self.assertEqual(
            data["reviews"][1]["submitted_timestamp"], "2018-06-06T00:00:00"
        )
        self.assertEqual(
            data["reviews"][2]["submitted_timestamp"], "2019-01-01T00:00:00"
        )
    ...
    def test_order_by_nationalitycountry(self):
        ...
        params = {
            ...
            "page_number": 0,
            "limit": 10,
            "sort_column": "nationality_country",
        }
        header = self.get_auth_header_for("r3@r.com")
    ...
    def test_order_by_department(self):
        ...
        self.assertEqual(
            data["reviews"][0]["department"], "CS"
        )  # ascii ordering orders capital letters before lowercase
    ...
    def test_review_form_language(self):
        ...
        self.assertEqual(
            data["review_form"]["review_sections"][0]["headline"],
            "Review Section 1 English",
        )
        self.assertEqual(
            data["review_form"]["review_sections"][0]["description"],
            "Review Section 1 Description English",
        )

        self.assertEqual(
            len(data["review_form"]["review_sections"][0]["review_questions"]), 2
        )

        self.assertEqual(
            data["review_form"]["review_sections"][0]["review_questions"][1][
                "description"
            ],
            "English Description",
        )
        ...
        self.assertDictEqual(
            data["review_form"]["review_sections"][0]["review_questions"][1]["options"][
                0
            ],
            {"label": "en1", "value": "en"},
        )
...
class ReviewListAPITest(ApiTestCase):
    def seed_static_data(self):
        ...
        self.add_review_section_translation(
            review_form1_section1.id, "en", "Review Section 1 en", "Review Section 1 en"
        )
        self.add_review_section_translation(
            review_form1_section1.id, "fr", "Review Section 1 fr", "Review Section 1 fr"
        )
        review_form1_section2 = self.add_review_section(review_form1.id)
        self.add_review_section_translation(
            review_form1_section2.id, "en", "Review Section 2 en", "Review Section 2 en"
        )
        self.add_review_section_translation(
            review_form1_section2.id, "fr", "Review Section 2 fr", "Review Section 2 fr"
        )

        review_form1_section2 = self.add_review_section(review_form1.id)
        review_q1 = self.add_review_question(review_form1_section1.id, weight=1)
        review_q1_translation_en = self.add_review_question_translation(
            review_q1.id, "en", headline="Heading En"
        )
        review_q1_translation_fr = self.add_review_question_translation(
            review_q1.id, "fr", headline="Heading Fr"
        )
        review_q2 = self.add_review_question(review_form1_section1.id, weight=0)
        review_q2_translation_en = self.add_review_question_translation(
            review_q2.id, "en", headline="Heading En"
        )
        review_q2_translation_fr = self.add_review_question_translation(
            review_q2.id, "fr", headline="Heading Fr"
        )
        review_q3 = self.add_review_question(review_form1_section2.id, weight=1)
        review_q3_translation_en = self.add_review_question_translation(
            review_q3.id, "en", headline="Heading En"
        )
        review_q3_translation_fr = self.add_review_question_translation(
            review_q3.id, "fr", headline="Heading Fr"
        )

        application_form2 = self.create_application_form(self.event2.id)
        review_form2 = self.add_review_form(application_form2.id)

        event1_response1 = self.add_response(
            application_form1.id, self.user1.id, is_submitted=True
        )
        ...
        event1_response2 = self.add_response(
            application_form1.id, user2.id, is_submitted=True, language="fr"
        )
        ...
        event1_response3 = self.add_response(
            application_form1.id, user3.id, is_submitted=True
        )
        ...
        event1_response4 = self.add_response(
            application_form1.id, user4.id, is_submitted=True, language="zu"
        )
        ...
        event2_response1 = self.add_response(
            application_form2.id, self.user1.id, is_submitted=True
        )
        event2_response2 = self.add_response(
            application_form2.id, user2.id, is_submitted=True, language="fr"
        )
        event2_response3 = self.add_response(
            application_form2.id, user3.id, is_submitted=True
        )

        # Review 1 completed review
        self.add_response_reviewer(event1_response1.id, self.reviewer1.id)
        review_response1 = self.add_review_response(
            self.reviewer1.id, event1_response1.id, review_form1.id
        )
        ...
        self.review_response1_submitted = (
            review_response1.submitted_timestamp.isoformat()
        )
        self.add_response_reviewer(event1_response2.id, self.reviewer1.id)
        review_response2 = self.add_review_response(
            self.reviewer1.id, event1_response2.id, review_form1.id
        )
...
class ResponseReviewerAssignmentApiTest(ApiTestCase):
    def seed_static_data(self):
        ...
        self.response1 = self.add_response(
            application_form.id, self.user1.id, is_submitted=True
        )
        self.response2 = self.add_response(
            application_form.id, self.user2.id, is_submitted=True
        )
        self.response3 = self.add_response(
            application_form.id, self.user3.id, is_submitted=True
        )

        self.add_review_form(application_form.id)
        self.add_review_form(application_form2.id)

        self.event2_response_id = self.add_response(
            application_form2.id, self.user1.id, is_submitted=True
        ).id
    ...
    def test_responses_assigned(self):
        ...
        response_reviewers = (
            ...
            .filter_by(application_form_id=1)
            .all()
        )
    ...
    def test_response_for_different_event_forbidden(self):
        ...
        params = {
            "event_id": 1,
            "response_ids": [1, 2, self.event2_response_id],
            "reviewer_email": "reviewer@mail.com",
        }
    ...
    def test_delete(self):
        ...
        response_reviewer = (
            db.session.query(ResponseReviewer)
            .filter_by(response_id=response_id, reviewer_user_id=self.reviewer_user_id)
            .first()
        )
...
class ReferenceReviewRequest(ApiTestCase):
    def static_seed_data(self):
        ...
        self.test_response = self.add_response(
            1, self.first_user.id, language="en", is_submitted=True
        )

        self.response_review = self.add_response_reviewer(
            self.test_response.id, self.first_user.id
        )
    ...
    def test_get_reference_request_by_event_id(self):
        ...
        response = self.app.get(
            "/api/v1/review",
            headers=self.get_auth_header_for("firstuser@mail.com"),
            data=params,
        )
    ...
    def test_get_reference_request_with_two_references(self):
        ...
        reference_req2 = ReferenceRequest(
            1, "Mrs", "Jane", "Jones", "Manager", "jane@email.com"
        )
        reference_request_repository.create(reference_req)
...
class ReviewResponseDetailListApiTest(ApiTestCase):
    def seed_static_data(self):
        ...
        self.question1 = self.add_question(
            self.application_form.id, self.section.id, order=1, key="review-identifier"
        )
        self.question_translation1 = self.add_question_translation(
            self.question1.id, "en", "Organisation"
        )
        self.question2 = self.add_question(
            self.application_form.id, self.section.id, order=2, key="review-identifier"
        )
        self.question_translation2 = self.add_question_translation(
            self.question2.id, "en", "Country"
        )
        ...
        self.question_translation3 = self.add_question_translation(
            self.question3.id, "en", "Non-review question"
        )

        self.response1 = self.add_response(
            self.application_form.id, self.user1.id, is_submitted=True
        )
        self.answer1 = self.add_answer(
            self.response1.id, self.question1.id, "Pets R Us"
        )
        self.answer2 = self.add_answer(self.response1.id, self.question2.id, "Nigeria")
        self.answer3 = self.add_answer(
            self.response1.id, self.question3.id, "Non-review answer"
        )

        self.response2 = self.add_response(
            self.application_form.id, self.user2.id, is_submitted=True
        )
        self.answer4 = self.add_answer(self.response2.id, self.question1.id, "Nokia")
        self.answer5 = self.add_answer(
            self.response2.id, self.question2.id, "South Africa"
        )
        self.answer6 = self.add_answer(
            self.response2.id, self.question3.id, "Non-review answer 2"
        )
        ...
        self.review_question1 = self.add_review_question(
            self.review_section1.id, type="short-text", weight=1
        )
        ...
        self.review_question2 = self.add_review_question(
            self.review_section1.id, type="long-text", weight=0
        )
        ...
        self.review_question3 = self.add_review_question(
            self.review_section2.id, type="multi-choice", weight=2
        )
        ...
        self.review_question4 = self.add_review_question(
            self.review_section2.id, type="checkbox", weight=0
        )
        self.review_question_translation4 = self.add_review_question_translation(
            self.review_question4.id, "en", headline="Yes/No Question"
        )

        self.review_response1 = self.add_review_response(
            self.reviewer.id, self.response1.id, self.review_form.id, is_submitted=True
        )
        self.review_score1 = self.add_review_score(
            self.review_response1.id, self.review_question1.id, "4"
        )
        self.review_score2 = self.add_review_score(
            self.review_response1.id,
            self.review_question2.id,
            "This is a very good proposal",
        )
        self.review_score3 = self.add_review_score(
            self.review_response1.id, self.review_question3.id, "5"
        )
        self.review_score4 = self.add_review_score(
            self.review_response1.id, self.review_question4.id, "Yes"
        )

        self.review_response2 = self.add_review_response(
            self.reviewer2.id, self.response2.id, self.review_form.id, is_submitted=True
        )
        self.review_score5 = self.add_review_score(
            self.review_response2.id, self.review_question1.id, "3"
        )
        self.review_score6 = self.add_review_score(
            self.review_response2.id, self.review_question2.id, "Not bad!"
        )
        self.review_score7 = self.add_review_score(
            self.review_response2.id, self.review_question3.id, "3"
        )
        self.review_score8 = self.add_review_score(
            self.review_response2.id, self.review_question4.id, "No"
        )
    ...
    def test_review_responses_for_event(self):
        ...
        self.assertEqual(
            data[0]["scores"][0]["description"],
            "How ethical is the candidate's proposal from 1 to 5?",
        )
        ...
        self.assertEqual(
            data[0]["scores"][1]["description"], "Comments for the candidate"
        )
        ...
        self.assertEqual(
            data[0]["scores"][2]["description"], "What is your overall rating?"
        )
        ...
        self.assertEqual(
            data[1]["scores"][1]["description"], "Comments for the candidate"
        )
        ...
        self.assertEqual(
            data[1]["scores"][2]["description"], "What is your overall rating?"
        )
...
class ReviewResponseSummaryListApiTest(ApiTestCase):
    def seed_static_data(self):
        ...
        self.review_response3 = self.add_review_response(
            self.reviewer2.id, self.response1.id, self.review_form.id, is_submitted=True
        )
        self.review_score9 = self.add_review_score(
            self.review_response3.id, self.review_question1.id, "3"
        )
        self.review_score10 = self.add_review_score(
            self.review_response3.id,
            self.review_question2.id,
            "This is a good proposal",
        )
        self.review_score11 = self.add_review_score(
            self.review_response3.id, self.review_question3.id, "4"
        )
        self.review_score12 = self.add_review_score(
            self.review_response3.id, self.review_question4.id, "Yes"
        )
        ...
        self.review_score6 = self.add_review_score(
            self.review_response2.id, self.review_question2.id, "Close to bad!"
        )
        self.review_score7 = self.add_review_score(
            self.review_response2.id, self.review_question3.id, "2"
        )
        self.review_score8 = self.add_review_score(
            self.review_response2.id, self.review_question4.id, "No"
        )
        ...
        self.review_response4 = self.add_review_response(
            self.reviewer.id, self.response2.id, self.review_form.id, is_submitted=True
        )
        self.review_score13 = self.add_review_score(
            self.review_response4.id, self.review_question1.id, "2"
        )
        self.review_score14 = self.add_review_score(
            self.review_response4.id, self.review_question2.id, "Bad!"
        )
        self.review_score15 = self.add_review_score(
            self.review_response4.id, self.review_question3.id, "1"
        )
        self.review_score16 = self.add_review_score(
            self.review_response4.id, self.review_question4.id, "No"
        )
    ...
    def test_review_response_summary_for_event(self):
        ...
        self.assertEqual(data[0]["scores"][1]["headline"], None)
        self.assertEqual(
            data[0]["scores"][1]["description"], "What is your overall rating?"
        )
        ...
        self.assertEqual(data[1]["scores"][1]["headline"], None)
        self.assertEqual(
            data[1]["scores"][1]["description"], "What is your overall rating?"
        )
...
REVIEW_FORM_POST = {
    ...
    "sections": [
            ...
            "questions": [
                {
                    ...
                    "weight": 0.0,
                    "question_id": 0,
                    "headline": {"en": "Question 1", "fr": "Question 1"},
                    "description": {
                    ...
                    "options": {"en": None, "fr": None},
                    "validation_regex": {"en": "(a|b|c|d)", "fr": "(a|b|c|d)"},
                    "validation_text": {
                ...
                },
                {
                    ...
                    "weight": 1.5,
                    "question_id": 0,
                    "headline": {"en": "Question 2", "fr": "Question 2"},
                    "description": {
                        "en": "This is the second question",
                        "fr": "C'est la deuxième question",
                    },
                    "placeholder": {"en": None, "fr": None},
                    "options": {
                        "en": [
                            {"value": "0", "label": "Bad"},
        ...
        {
            "order": 2,
            "name": {"en": "Section 2", "fr": "Section 2"},
            "description": {
                "en": "This is the first section",
...
REVIEW_FORM_PUT = {
    ...
    "sections": [
        {  # Updated first section
            "id": 1,
            "order": 2,  # Updated
            "name": {"en": "Section 1 Updated", "fr": "Section 1 Updated"},
            "description": {
            ...
            "questions": [
                {
                    ...
                    "options": {"en": {"blah": "blah"}, "fr": {"blah": "blah"}},
                    "validation_regex": {"en": "(a|b|c|d|e)", "fr": "(a|b|c|d|e)"},
                    "validation_text": {
                ...
                {  # New question
                    ...
                    "weight": 2.5,
                    "question_id": 0,
                    "headline": {"en": "Question 3", "fr": "Question 3"},
                    "description": {
                        "en": "This is the third question",
                        "fr": "C'est la deuxième question",
                    },
                    "placeholder": {"en": "hello", "fr": "world"},
                    "options": {
                        "en": [
                            {"value": "0", "label": "Bad"},
                            {"value": "1", "label": "Average"},
        ...
        {  # New section
            "order": 3,
            "name": {"en": "Section 2 New", "fr": "Section 2 New"},
            "description": {
...
class ReviewFormDetailAPITest(ApiTestCase):
    ...
    def add_review_forms(self):
        ...
        form1 = self.add_review_form(
            stage=1, active=False, deadline=datetime(2021, 6, 1)
        )
        ...
        self.add_review_section_translation(
            ...
            "en",
            "Form 1 Section 1 EN",
            "Form 1 Section 1 Description EN",
        )
        self.add_review_section_translation(
            form1_section1.id,
            "fr",
            "Form 1 Section 1 FR",
            "Form 1 Section 1 Description FR",
        )

        form1_section1_question1 = self.add_review_question(
            form1_section1.id, weight=1, type="numeric-text"
        )
        ...
        form1_section1_question2 = self.add_review_question(
            form1_section1.id, type="information", order=2, question_id=self.question.id
        )
        self.add_review_question_translation(
        ...
        self.add_review_section_translation(
            ...
            "en",
            "Form 1 Section 2 EN",
            "Form 1 Section 2 Description EN",
        )
        self.add_review_section_translation(
            form1_section2.id,
            "fr",
            "Form 1 Section 2 FR",
            "Form 1 Section 2 Description FR",
        )

        form1_section2_question1 = self.add_review_question(
            form1_section2.id, weight=1, type="radio"
        )
        self.add_review_question_translation(
        ...
        form2 = self.add_review_form(
            stage=2, active=True, deadline=datetime(2021, 8, 1)
        )
        ...
        self.add_review_section_translation(
            ...
            "en",
            "Form 2 Section 1 EN",
            "Form 2 Section 1 Description EN",
        )
        self.add_review_section_translation(
            form2_section1.id,
            "fr",
            "Form 2 Section 1 FR",
            "Form 2 Section 1 Description FR",
        )

        form2_section1_question1 = self.add_review_question(
            form2_section1.id, type="information", question_id=self.question.id
        )
        self.add_review_question_translation(
    ...
    def test_get_first_stage(self):
        ...
        self.assertEqual(
            section1["description"]["en"], "Form 1 Section 1 Description EN"
        )
        self.assertEqual(
            section1["description"]["fr"], "Form 1 Section 1 Description FR"
        )
        ...
        self.assertEqual(
            question1["description"]["en"], "Review Question 1 Description EN"
        )
        self.assertEqual(
            question1["description"]["fr"], "Review Question 1 Description FR"
        )
        ...
        self.assertEqual(
            question1["validation_text"]["en"], "Validation text question 1 EN"
        )
        self.assertEqual(
            question1["validation_text"]["fr"], "Validation text question 1 FR"
        )
        ...
        self.assertEqual(
            question2["description"]["en"], "Review Question 2 Description EN"
        )
        self.assertEqual(
            question2["description"]["fr"], "Review Question 2 Description FR"
        )
        ...
        self.assertEqual(
            section2["description"]["en"], "Form 1 Section 2 Description EN"
        )
        self.assertEqual(
            section2["description"]["fr"], "Form 1 Section 2 Description FR"
        )
        ...
        self.assertEqual(
            question3["description"]["en"], "Review Question 3 Description EN"
        )
        self.assertEqual(
            question3["description"]["fr"], "Review Question 3 Description FR"
        )
    ...
    def test_get_second_stage(self):
        ...
        self.assertEqual(
            section1["description"]["en"], "Form 2 Section 1 Description EN"
        )
        self.assertEqual(
            section1["description"]["fr"], "Form 2 Section 1 Description FR"
        )
        ...
        self.assertEqual(
            question1["description"]["en"], "Stage 2 Review Question 1 Description EN"
        )
        self.assertEqual(
            question1["description"]["fr"], "Stage 2 Review Question 1 Description FR"
        )
    ...
    def test_post(self):
        ...
        response = self.app.get(
            ...
            headers=self.get_auth_header_for("event@admin.com"),
            data={
                "event_id": REVIEW_FORM_POST["event_id"],
                "stage": REVIEW_FORM_POST["stage"],
            },
        )
        ...
        self.assertTrue(
            "id" in data["sections"][0] and data["sections"][0]["id"] is not None
        )
        del data["sections"][0]["id"]

        self.assertTrue(
            "id" in data["sections"][1] and data["sections"][1]["id"] is not None
        )
        del data["sections"][1]["id"]

        self.assertTrue(
            "id" in data["sections"][0]["questions"][0]
            and data["sections"][0]["questions"][0]["id"] is not None
        )
        del data["sections"][0]["questions"][0]["id"]

        self.assertTrue(
            "id" in data["sections"][0]["questions"][1]
            and data["sections"][0]["questions"][1]["id"] is not None
        )
        del data["sections"][0]["questions"][1]["id"]

        self.assertTrue(
            "id" in data["sections"][1]["questions"][0]
            and data["sections"][1]["questions"][0]["id"] is not None
        )
