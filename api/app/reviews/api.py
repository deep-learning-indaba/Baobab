...
from app.reviews.mixins import (
    GetReviewAssignmentMixin,
    GetReviewHistoryMixin,
    GetReviewResponseMixin,
    GetReviewSummaryMixin,
    PostReviewAssignmentMixin,
    PostReviewResponseMixin,
    ReviewMixin,
)
...
from app.utils.errors import (
    ...
    REVIEW_FORM_NOT_FOUND,
    REVIEW_RESPONSE_NOT_FOUND,
    USER_NOT_FOUND,
)

option_fields = {"value": fields.String, "label": fields.String}
...
review_scores_fields = {"review_question_id": fields.Integer, "value": fields.String}
...
review_response_fields = {
    ...
    "scores": fields.List(
        fields.Nested(review_scores_fields), attribute="review_scores"
    ),
...
review_section_detail_fields = {
    ...
    "questions": fields.List(
        fields.Nested(review_question_detail_fields), attribute="review_questions"
    ),
}
...
review_form_detail_fields = {
    ...
    "sections": fields.List(
        fields.Nested(review_section_detail_fields), attribute="review_sections"
    ),
}
...
class ReviewAPI(ReviewMixin, restful.Resource):
    ...
    def get(self):
        ...
        review_response = (
            None
            if response is None
            else review_repository.get_review_response(
...
class ResponseReviewAPI(restful.Resource):
    ...
    def get(self):
        ...
        return ReviewResponseUser(
            review_form, response, 0, args["language"], review_response=review_response
        )


class ReviewResponseAPI(
    GetReviewResponseMixin, PostReviewResponseMixin, restful.Resource
):
    @auth_required
    @marshal_with(review_fields)
    def get(self):
    ...
    def post(self):
        ...
        review_response = ReviewResponse(
            review_form_id, reviewer_user_id, response_id, language
        )
    ...
    def get_error_message(self, key):
        return (
            {
                "message": {
                    key: "Missing required parameter in the JSON body or the post body or the query string"
                }
            },
            400,
        )
...
class ReviewAssignmentAPI(
    GetReviewAssignmentMixin, PostReviewAssignmentMixin, restful.Resource
):
    ...
    def post(self):
        ...
        reviewer_user = user_repository.get_by_email(
            reviewer_user_email, g.organisation.id
        )
        ...
        response_ids = self.get_eligible_response_ids(
            event_id, reviewer_user.id, num_reviews, config.num_reviews_required
        )
        response_reviewers = [
    ...
    def get_eligible_response_ids(
        self, event_id, reviewer_user_id, num_reviews, reviews_required
    ):
...
class ReviewListAPI(restful.Resource):
    ...
    def _serialize_answer(answer, language):
        ...
        return {"headline": translation.headline, "value": answer.value_display}
    ...
    def _serialize_response(response, review_response, language):
        info = [
            ...
            if answer.question.key == "review-identifier"
        ]
        ...
        return {
            ...
            "submitted": submitted,
            "total_score": review_response.calculate_score()
    ...
    def get(self):
        ...
        return [
            ReviewListAPI._serialize_response(response, review_response, language)
...
class ReviewResponseDetailListAPI(restful.Resource):
    ...
    def _serialise_score(review_score, language):
        ...
        if review_question_translation is None:
            ...
            LOGGER.warn(
                "Could not find {} translation for review question id {}".format(
                    language, review_score.review_question.id
                )
            )
    ...
    def _serialise_review_response(review_response, language):
        return {
            ...
            "scores": [
                ...
                if (
                    review_score.review_question.type == "multi-choice"
        ...
        for review_section in review_form.review_sections:
            for review_question in review_section.review_questions:
                if review_question.weight > 0:
                    ...
                    average_score = (
                        review_repository.get_average_score_for_review_question(
...
class ReviewStageAPI(restful.Resource):
    ...
    def get(self):
        ...
        return {"current_stage": current_form.stage, "total_stages": len(review_forms)}
        ...
        for section_data in sections_data:
            if "id" in section_data:
                ...
                section = next(
                    (
                        s
                        for s in review_form.review_sections
                ...
                )  # type: Sequence[ReviewSectionTranslation]
                for current_translation in current_translations:
                    current_translation.description = section_data["description"][
            ...
            for question_data in section_data["questions"]:
                if "id" in question_data:
                    ...
                    current_translations = (
                        current_question.translations
                    )  # type: Sequence[ReviewQuestionTranslation]
                    for current_translation in current_translations:
                        current_translation.description = question_data["description"][
                            current_translation.language
                        ]
                        current_translation.headline = question_data["headline"][
                            current_translation.language
                        ]
                        current_translation.options = question_data["options"][
                            current_translation.language
                        ]
                        current_translation.placeholder = question_data["placeholder"][
                            current_translation.language
                        ]
