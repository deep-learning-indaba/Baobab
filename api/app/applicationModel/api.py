...
class ApplicationFormAPI(restful.Resource):
    ...
    def get(self):
        ...
        try:
            ...
            if not event.is_application_open and not user_repository.get_by_id(
                g.current_user["id"]
            ).is_event_admin(args["event_id"]):
...
class ApplicationFormDetailAPI(restful.Resource):
    ...
    def post(self, event_id):
        ...
        for section_data in sections_data:
            section = Section(
                app_form.id, section_data["order"], key=section_data["key"]
            )
    ...
    def put(self, event_id):
        ...
        all_question_ids = [
            q["id"] for s in incoming_sections for q in s["questions"] if "id" in q
        ]
        ...
        for section_data in incoming_sections:
            if "id" in section_data:
                ...
                section = next(
                    (s for s in current_sections if s.id == section_data["id"]), None
                )  # type: Section
                if not section:
                    return SECTION_NOT_FOUND
                ...
                )  # type: Sequence[SectionTranslation]
    ...
    for question_data in questions:
        if "id" in question_data:
            current_question = next(
                (s for s in current_questions if s.id == question_data["id"]), None
            )  # type: Question
            ...
            current_translations = (
                current_question.question_translations
            )  # type: Sequence[QuestionTranslation]
            for current_translation in current_translations:
                ...
                current_translation.show_for_values = question_data["show_for_values"][
                    current_translation.language
                ]
                current_translation.validation_regex = question_data[
                    "validation_regex"
                ][current_translation.language]
                current_translation.validation_text = question_data["validation_text"][
                    current_translation.language
                ]
...
def _serialize_question(question, language):
    ...
    return dict(
        question_id=question.id, headline=translation.headline, type=question.type
    )
