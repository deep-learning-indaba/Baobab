...
class GuestRegistrationRepository:
    ...
    def get_all_unsigned_guests(event_id):
        ...
        return (
            db.session.query(InvitedGuest, AppUser, GuestRegistration)
    ...
    def get_confirmed_guest_for_event(event_id, confirmed):
        ...
        return (
            db.session.query(GuestRegistration, InvitedGuest, AppUser)
            .filter(GuestRegistration.confirmed == confirmed)
            .join(InvitedGuest, InvitedGuest.user_id == GuestRegistration.user_id)
            .filter(InvitedGuest.event_id == event_id)
            .join(AppUser, InvitedGuest.user_id == AppUser.id)
            .all()
        )
    ...
    def count_guests(event_id):
        count = db.session.query(InvitedGuest).filter_by(event_id=event_id).count()
        return count
    ...
    def timeseries_guest_registrations(event_id):
        timeseries = (
            db.session.query(
                cast(GuestRegistration.created_at, Date),
                func.count(GuestRegistration.created_at),
            )
            .join(
                RegistrationForm,
                GuestRegistration.registration_form_id == RegistrationForm.id,
