...
class EventsAPITest(ApiTestCase):
    ...
    def test_get_events_applied(self):
        ...
        response = self.app.post(
            "/api/v1/authenticate",
            data={"email": "something@email.com", "password": "abc"},
        )
    ...
    def test_past_event_non_attendee(self):
        ...
        past_event1 = self.add_event(
            start_date=datetime.now() - timedelta(days=30),
            end_date=datetime.now() - timedelta(days=30),
            key="PAST1",
        )

        past_event2 = self.add_event(
            start_date=datetime.now() - timedelta(days=30),
            end_date=datetime.now() - timedelta(days=30),
            key="PAST2",
        )
...
class EventsStatsAPITest(ApiTestCase):
    ...
    def seed_static_data(self):
        ...
        self.test_form = self.create_application_form(self.test_event.id, True, False)
        db.session.add(self.test_form)
        db.session.commit()

        self.test_response = self.add_response(self.test_form.id, self.test_user1["id"])
        self.test_response2 = self.add_response(
            self.test_form.id, self.test_user2["id"], is_submitted=True
        )
...
class RemindersAPITest(ApiTestCase):
    def seed_static_data(self):
        ...
        email_templates = [
            EmailTemplate(
                "application-not-submitted", None, "Application Not Submitted", "", "en"
            ),
            EmailTemplate(
                "application-not-started", None, "Application Not Started", "", "en"
            ),
        ]
        ...
        submitted_response = self.add_response(
            application_form.id, self.test_users[0].id, is_submitted=True
        )
        withdrawn_response = self.add_response(
            application_form.id, self.test_users[3].id, is_withdrawn=True
        )
...
class EventAPITest(ApiTestCase):
    ...
    test_event_data_dict = {
        "name": {"en": "Test Event", "fr": "evenement de test"},
        "description": {
    ...
    def seed_static_data(self):
        ...
        self.test_admin_user = AppUser(
            email="someoneadmin@email.com",
            firstname="Some",
            lastname="One",
            user_title="Mr",
            password="abc",
    ...
    def test_post_event_is_admin(self):
        ...
        self.assertEqual(
            data["description"]["fr"], "Description de l'evenement de test"
        )
    ...
    def test_post_event_eventrole_added(self):
        ...
        body = {"email": self.test_admin_user.email, "password": "abc"}
        response = self.app.post("api/v1/authenticate", data=body)
        data = json.loads(response.data)
    ...
    def test_put_event_is_admin(self):
        ...
        self.test_event_data_dict["description"] = {
            "en": "Test Event Description Updated"
        }
        response = self.app.put(
            "api/v1/event",
            headers=header,
...
class EventStatusTest(ApiTestCase):
    ...
    def test_invited_guest_unregistered(self):
        ...
        invited = InvitedGuest(
            event_id=self.event.id, user_id=self.user1.id, role="Mentor"
        )
    ...
    def test_outcome(self):
        ...
        outcome = Outcome(
            self.event.id, self.user1.id, OutcomeStatus.ACCEPTED, self.user2.id
        )
