...
from app.events.mixins import EventMixin, EventsKeyMixin, EventsMixin
from app.events.models import Event, EventRole, EventType
from app.events.repository import EventRepository as event_repository
from app.guestRegistrations.repository import (
    GuestRegistrationRepository as guest_registration_repository,
)
from app.organisation.models import Organisation
from app.registration.repository import OfferRepository as offer_repository
from app.registration.repository import (
    RegistrationRepository as registration_repository,
)
from app.responses.models import Response
from app.responses.repository import ResponseRepository as response_repository
from app.reviews.repository import (
    ReviewConfigurationRepository as review_config_repository,
)
...
event_fields = {
    "id": fields.Integer,
    "name": fields.Raw(
        attribute=lambda event: event.get_all_name_translations()
        if isinstance(event, Event)
        else {}
    ),
    "description": fields.Raw(
        attribute=lambda event: event.get_all_description_translations()
        if isinstance(event, Event)
        else {}
    ),
    "key": fields.String,
...
def get_user_event_response_status(user_id, event_id):
    def _log_application_status(context):
        LOGGER.debug(
            "Application {} for user_id: {}, event_id: {}".format(
    ...
    try:
        applicationForm = (
            db.session.query(ApplicationForm)
        ...
        if applicationForm:
            if applicationForm.event.application_close < datetime.now():
                ...
                response = (
                    ...
                    .order_by(Response.started_timestamp.desc())
                    .first()
                )
    ...
    def get(self):
        ...
        for event in itertools.chain(upcoming_events, attended_events):
            if not event.has_specific_translation(language):
                LOGGER.error(
                    "Missing {} translation for event {}.".format(language, event.id)
                )
                language = default_language
            status = (
                None
                if user_id == 0
                else event_status.get_event_status(event.id, user_id)
            )
...
def _process_timeseries(timeseries):
    timeseries = [(d.strftime("%Y-%m-%d"), c) for (d, c) in timeseries if d is not None]
    return timeseries
...
class EventStatsAPI(EventsMixin, restful.Resource):
    ...
    def get(self, event_id):
        ...
        required_reviews = (
            1 if review_config is None else review_config.num_reviews_required
        )
        ...
        reviews_timeseries = _process_timeseries(
            review_repository.get_review_complete_timeseries_by_event(event_id)
        )
        ...
        guest_registration_timeseries = _process_timeseries(
            guest_registration_repository.timeseries_guest_registrations(event_id)
        )
        ...
        for user in users:
            ...
            email_user(
                ...
                template_parameters=dict(
                    organisation_name=organisation_name, deadline=deadline
                ),
                event=event,
                user=user,
            )
