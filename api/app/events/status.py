...
from app.invitedGuest.repository import (
    InvitedGuestRepository as invited_guest_repository,
)
...
from app.registrationResponse.repository import (
...
class EventStatus:
    ...
    def __init__(
        ...
        if (
            invited_guest is None
            and registration_status is not None
            and offer_status is None
        ):
            raise ValueError(
                "Non-invited guest must have an offer to have a registration status"
            )

        if (
            invited_guest is None
            and outcome_status is not None
