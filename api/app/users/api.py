...
from app.users.mixins import (
    AuthenticateMixin,
    EventAttendeeMixin,
    PrivacyPolicyMixin,
    SignupMixin,
    UserProfileListMixin,
    UserProfileMixin,
)
...
def user_info(user, roles):
    return {
        ...
        "roles": [
            {"event_id": event_role.event_id, "role": event_role.role}
            for event_role in roles
        ],
    }
...
class UserAPI(SignupMixin, restful.Resource):
    ...
    def get(self):
        user = (
            db.session.query(AppUser).filter(AppUser.id == g.current_user["id"]).first()
        )
        return user
...
class UserProfileList(UserProfileListMixin, restful.Resource):
    ...
    def get(self):
        ...
        user_responses = user_repository.get_all_with_responses_or_invited_guests_for(
            event_id
        )
...
class PasswordResetRequestAPI(restful.Resource):
    def post(self):
        ...
        LOGGER.debug(
            "Requesting password reset for email {} and organisation {}".format(
...
class VerifyEmailAPI(restful.Resource):
    def get(self):
        ...
        user = db.session.query(AppUser).filter(AppUser.verify_token == token).first()
...
class UserCommentAPI(restful.Resource):
    ...
    def get(self):
        ...
        comments = (
            db.session.query(UserComment)
            .filter(
                UserComment.event_id == args["event_id"],
                UserComment.user_id == args["user_id"],
            )
            .all()
        )
