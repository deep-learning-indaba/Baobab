...
from app.registrationResponse.mixins import (
    RegistrationAdminMixin,
    RegistrationConfirmMixin,
...
class RegistrationApi(RegistrationResponseMixin, restful.Resource):
    ...
    def get(self):
        ...
        try:
            ...
            registration = (
                ...
                .filter(Registration.offer_id == db_offer.id)
                .first()
            )
            ...
            db_answers = (
                ...
                .filter(RegistrationAnswer.registration_id == registration.id)
                .all()
            )
    ...
    def post(self):
        ...
        try:
            ...
            current_user = (
                db.session.query(AppUser).filter(AppUser.id == user_id).first()
            )

            registration_form = (
                db.session.query(RegistrationForm)
                .filter(RegistrationForm.id == args["registration_form_id"])
                .first()
            )
    ...
    def put(self):
        ...
        try:
            ...
            registration = (
                db.session.query(Registration)
                .filter(Registration.id == args["registration_id"])
                .one_or_none()
            )
            if registration is None:
            ...
            db_offer = (
                ...
                .filter(Offer.id == registration.offer_id)
                .one_or_none()
            )
            ...
            for answer_args in args["answers"]:
                answer = (
                    ...
                    .filter(
                        ...
                        == answer_args["registration_question_id"],
                        RegistrationAnswer.registration_id == args["registration_id"],
                    )
                    .one_or_none()
                )
    ...
    def send_confirmation(self, user, questions, answers, confirmed, event):
        ...
        if questions is None:
            LOGGER.warn(
                "Found no questions associated with application form with id {form_id}".format(
                    form_id=user.id
                )
            )
        try:
                ...
                for question in questions:
                    if answer.registration_question_id == question.id:
                        summary += (
                            ...
                            + question.headline
                            + "\nAnswer:"
                            + _get_answer_value(answer, question)
                            + "\n"
                        )
            ...
            LOGGER.error(
                "Could not send confirmation email for response with id : {response_id}".format(
                    response_id=user.id
                )
            )
...
def map_registration_info_guests(registration_info):
    if (
        registration_info
        and registration_info.GuestRegistration
        and registration_info.GuestRegistration.id
    ):
...
def _get_registrations(event_id, user_id, confirmed, exclude_already_signed_in=False):
    try:
        ...
        if exclude_already_signed_in == True:
            ...
            guest_registration = GuestRegistrationRepository.get_all_guests(event_id)
        ...
        for name, group in itertools.groupby(
            sorted(all_registrations, key=lambda d: d["user_id"]),
...
class RegistrationConfirmedAPI(RegistrationAdminMixin, restful.Resource):
    ...
    def get(self):
        ...
        return _get_registrations(
            event_id,
            user_id,
            confirmed=None,
            exclude_already_signed_in=exclude_already_signed_in,
        )
...
class RegistrationConfirmAPI(RegistrationConfirmMixin, restful.Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if _send_registration_confirmation_mail(
                registration_user, registration_event
            ):
                registration.confirmation_email_sent_at = datetime.now()

            db.session.commit()
            return (
                "Confirmed Registration for {} {}".format(
                    registration_user.firstname, registration_user.lastname
                ),
                200,
            )
            ...
            LOGGER.error(
                "Error occured while confirming registration with id {}: {}".format(
                    registration_id, e
                )
            )
            return errors.DB_NOT_AVAILABLE
