...
class EmailerTest(ApiTestCase):
    ...
    def seed_static_data(self):
        ...
        self.english_user = self.add_user(
            email="english@person.com",
            post_create_fn=partial(set_language, language="en"),
        )
        self.french_user = self.add_user(
            email="french@person.com",
            post_create_fn=partial(set_language, language="fr"),
        )
        self.zulu_user = self.add_user(
            email="zulu@person.com", post_create_fn=partial(set_language, language="zu")
        )
        ...
        self.add_email_template(
            "template1",
            "English template no event {param}",
            subject="English subject no event",
        )
        self.add_email_template(
            "template1",
            "Modèle français sans événement {param}",
            subject="Sujet français sans événement",
            language="fr",
        )
        self.add_email_template(
            "template1",
            "English template {event_name} {param}",
            subject="English subject {event_name}",
            event_id=1,
        )
        self.add_email_template(
            "template1",
            "Modèle français {event_name} {param}",
            subject="Sujet français {event_name}",
            language="fr",
            event_id=1,
        )
    ...
    def test_email_no_event_english(self, send_mail_fn):
        ...
        email_user(
            "template1", template_parameters={"param": "Blah"}, user=self.english_user
        )
    ...
    def test_email_no_event_french(self, send_mail_fn):
        ...
        email_user(
            "template1", template_parameters={"param": "bleu"}, user=self.french_user
        )
    ...
    def test_email_english_default(self, send_mail_fn):
        ...
        email_user(
            "template1", template_parameters={"param": "Blah"}, user=self.zulu_user
        )
