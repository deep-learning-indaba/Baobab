...
def build_response_email_body(answers, language, application_form):
    ...
    allQuestions = [
        q for section in application_form.sections for q in section.questions
    ]
    ...
    for section in application_form.sections:
        ...
        for question in section.questions:
            ...
            if question.depends_on_question_id and question_translation.show_for_values:
                dependency_question = _find_question(
                    question.depends_on_question_id, allQuestions
                )
                dependency_answer = _find_answer(dependency_question, answers)
                if (
                    dependency_answer
                    and dependency_answer not in question_translation.show_for_values
                ):
                    continue
