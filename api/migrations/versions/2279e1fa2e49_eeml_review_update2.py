...
def update_review_question_order(session, headline, new_order):
    ...
    if review_question is None:
        review_question = session.query(ReviewQuestion).join(Question, ReviewQuestion.question_id == Question.id).filter(Question.headline == headline).first()
    
    review_question.order = new_order
        

def delete_review_question(session, form_id, headline):
    session.query(ReviewQuestion).filter_by(review_form_id=form_id, headline=headline).delete()
...
def upgrade():
    ...
    divider1 = ReviewQuestion(form.id, 'section-divider', False, 1, headline='Candidate Application')
    session.add(divider1)

    category_question = find_question(session, 'I Am:')
    category_info = ReviewQuestion(form.id, 'information', False, 2, question_id=category_question.id, headline='Applicant Category')
    ...
    country_info = ReviewQuestion(form.id, 'information', False, 4, question_id=country_question.id, headline='Country')
    session.add(country_info)

    research_group_question = find_question(session, 'Research Group / Laboratory')
    research_group_info = ReviewQuestion(form.id, 'information', False, 5, question_id=research_group_question.id, headline='Research Group / Laboratory')
    session.add(research_group_info)

    supervisor_question = find_question(session, 'Supervisor')
    supervisor_info = ReviewQuestion(form.id, 'information', False, 6, question_id=supervisor_question.id, headline='Supervisor')
    session.add(supervisor_info)

    previous_attendance_question = find_question(session, 'Previous attendance')
    previous_attendance_info = ReviewQuestion(form.id, 'information', False, 7, question_id=previous_attendance_question.id, headline='Previous attendance')
    ...
    session.query(ReviewQuestion).filter_by(headline='Financial Support Motivation').delete()

    divider2 = ReviewQuestion(form.id, 'section-divider', False, 12, headline='Your Review')
    session.add(divider2)

    project = ReviewQuestion(form.id, 'long-text', True, 13, description='Please describe in 2-3 sentences what the project is about', headline='Project')
    session.add(project)

    update_review_question_order(session, 'Would this person benefit from attending the school?', 14)
    update_review_question_order(session, 'Would the school benefit from this person''s participation?', 15)
