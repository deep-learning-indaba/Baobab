...
def upgrade():
    ...
    affiliation_info = ReviewQuestion(
        form.id,
        "information",
        False,
        1,
        question_id=affiliation_question.id,
        headline="Affiliation",
    )

    research_interests_question = find_question(session, "Research Interests")
    research_interests_info = ReviewQuestion(
        form.id,
        "information",
        False,
        2,
        question_id=research_interests_question.id,
        headline="Research Interests",
    )

    extended_abstract_type = find_question(session, "Extended abstract type")
    extended_abstract_type_info = ReviewQuestion(
        form.id, "information", False, 3, question_id=extended_abstract_type.id
    )

    extended_abstract_question = find_question(session, "Extended abstract")
    extended_abstract_info = ReviewQuestion(
        form.id, "information", False, 4, question_id=extended_abstract_question.id
    )

    cv_question = find_question(session, "CV")
    cv_info = ReviewQuestion(
        form.id, "information", False, 5, question_id=cv_question.id, headline="CV"
    )
...
""",
        headline="Would the school benefit from this person" "s participation?",
        options=ten_scale,
    )
