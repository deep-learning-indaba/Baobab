...
def upgrade():
    ...
    op.add_column(
        "event", sa.Column("email_from", sa.String(length=255), nullable=True)
    )
    ...
    op.create_foreign_key(
        "fk_event_organisation", "event", "organisation", ["organisation_id"], ["id"]
    )
    op.add_column(
        "organisation", sa.Column("url", sa.String(length=100), nullable=True)
    )

    op.execute(
        """UPDATE organisation SET url = 'http://www.deeplearningindaba.com' WHERE id = 1"""
    )
    op.execute("""UPDATE organisation SET url = 'http://eeml.eu' WHERE id = 2""")
    op.execute(
        """UPDATE organisation SET url = 'http://www.deeplearningindaba.com' WHERE id = 3"""
    )

    op.execute(
        """UPDATE event SET email_from = 'baobab@deeplearningindaba.com', key='indaba2019', organisation_id=1, url='www.deeplearningindaba.com' WHERE id = 1"""
    )
