...
def upgrade():
    op.add_column('organisation', sa.Column('system_url', sa.String(length=100), nullable=True))

    op.execute("""UPDATE organisation SET system_url = 'test-baobab.deeplearningindaba.com' WHERE id = 1""")
    op.execute("""UPDATE organisation SET system_url = 'test-portal.eeml.eu' WHERE id = 2""")
