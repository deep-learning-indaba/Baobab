...
def upgrade():
    op.add_column(
        "organisation",
        sa.Column("privacy_policy", sa.String(length=100), nullable=True),
    )
    op.execute(
        """UPDATE organisation set privacy_policy='PrivacyPolicy.pdf' WHERE id in (1, 3)"""
    )
    op.execute(
        """UPDATE organisation set privacy_policy='EEML_PrivacyPolicy.pdf' WHERE id = 2"""
    )
