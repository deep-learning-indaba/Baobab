...
class RegistrationSection(Base):
    ...
    registration_form_id = db.Column(
        db.Integer(), db.ForeignKey("registration_form.id"), nullable=False
    )
    ...
    def __init__(
        ...
        description,
        order,
        show_for_travel_award,
        show_for_accommodation_award,
...
class RegistrationQuestion(Base):
    ...
    section_id = db.Column(
        db.Integer(), db.ForeignKey("registration_section.id"), nullable=False
    )
...
def upgrade():
    ...
    mentorship_q3 = RegistrationQuestion(
        ...
        None,
        is_required=False,
        description="e.g. expert from academia or industry, preferred gender/race of the expert for you to be more comfortable in the discussion etc.",
    )
    ...
    background = RegistrationSection(
        form.id, "Your Background", "", 2, None, None, None
    )
    ...
    poster_q1 = RegistrationQuestion(
        form.id, poster.id, "Title", "Title", 1, "short-text", None, is_required=False
    )
    poster_q2 = RegistrationQuestion(
        form.id,
        poster.id,
        "Authors",
        "Authors",
    ...
    poster_q5 = RegistrationQuestion(
        form.id, poster.id, "Teaser image", "", 5, "file", None, is_required=False
    )
    poster_q6 = RegistrationQuestion(
        form.id,
        poster.id,
        "Topic",
        "Choose an option",
    ...
    poster_q7 = RegistrationQuestion(
        form.id,
        poster.id,
    ...
    other_q1 = RegistrationQuestion(
        ...
        "multi-choice",
        None,
        description="Please check the website https://www.eeml.eu for a complete list of sponsors and partners",
        options=[
            {"value": "yes", "label": "Yes"},
            {
                "value": "no",
                "label": "No or Does not apply (e.g. you represent a sponsor or you did not submit your CV)",
            },
        ],
    )
...
def downgrade():
    ...
    if form:
        op.execute(
            """DELETE FROM registration_question WHERE registration_form_id={}""".format(
                form.id
            )
        )
        op.execute(
            """DELETE FROM registration_section WHERE registration_form_id={}""".format(
                form.id
            )
        )
