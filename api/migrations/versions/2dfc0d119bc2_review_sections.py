...
def upgrade():
    ...
    op.create_table('review_section_translation',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('review_section_id', sa.Integer(), nullable=False),
    sa.Column('language', sa.String(length=2), nullable=False),
    sa.Column('headline', sa.String(), nullable=True),
    sa.Column('description', sa.String(), nullable=True),
    sa.ForeignKeyConstraint(['review_section_id'], ['review_section.id'], ),
    sa.PrimaryKeyConstraint('id'),
    sa.UniqueConstraint('review_section_id', 'language', name='uq_review_section_id_language')
    )

    op.add_column('review_question', sa.Column('review_section_id', sa.Integer(), nullable=True))
    op.drop_constraint('review_question_review_form_id_fkey', 'review_question', type_='foreignkey')
    ...
    op.create_foreign_key('review_question_review_section_id_fkey', 'review_question', 'review_section', ['review_section_id'], ['id'])
...
def downgrade():
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('review_question', sa.Column('review_form_id', sa.INTEGER(), autoincrement=False, nullable=True))
    ...
    op.create_foreign_key('review_question_review_form_id_fkey', 'review_question', 'review_form', ['review_form_id'], ['id'])
