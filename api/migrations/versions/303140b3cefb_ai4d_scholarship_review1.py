...
def upgrade():
    ...
    divider1 = add_divider(form.id, {'en': 'Application', 'fr': 'Application'}, descriptions=None)

    sections = db.session.query(Section).filter_by(application_form_id=application_form.id).all()
    for section in sections:
        en = section.get_translation('en')
        if en.name == 'Organisation and Lead Applicant Contact details' or en.name == 'AI4D Call for Proposals: Scholarships program manager':
            continue
        add_divider(form.id, {'en': en.name, 'fr': section.get_translation('fr').name}, None, sub_heading=True)
        for question in section.questions:
            ...
            if question.type == 'multi-file':
                ...
                q_type = 'information'
            add_information_question(form.id, application_form.id, translation.headline, type=q_type)
    ...
    }, None, sub_heading=True)

    add_evaluation_question(form.id, None, {
...
""", 'fr': """"""
    ...
    add_comment_question(form.id, None, {
...
""", 'fr': """"""
    ...
    add_divider(form.id, {'en': '2. Scholarship Plan Proposal', 'fr': '2. Scholarship Plan Proposal'}, None, sub_heading=True)
...
""", 'fr': """"""
    ...
    add_divider(form.id, {'en': '3. Complementary activities plan', 'fr': '3. Complementary activities plan'}, None, sub_heading=True)
...
""", 'fr': """"""
    ...
    add_divider(form.id, {
        'en': """4. Marketing and communications approach""",
        'fr': """"""
    }, None, sub_heading=True)
...
**10 - Very Good**: meets and exceeds criteria """, 'fr': """"""
    }, 0, 10)
    ...
    add_divider(form.id, {'en': """5. Final Overview""", 'fr': """5. Final Overview"""}, None, sub_heading=True)

    add_question(form.id, 'multi-choice', True, descriptions={
    ...
    add_comment_question(form.id, None, {
        'en': """5.3 Feedback for applicant:""",
        'fr': """"""
    }, True)

    en, q = (db.session.query(QuestionTranslation, Question)
                       .filter_by(headline='Name of Organisation', language='en')
                       .join(Question, QuestionTranslation.question_id == Question.id)
                       .filter_by(application_form_id=application_form.id).first())
...
def downgrade():
    ...
    db.session.query(ReviewForm).filter_by(application_form_id=application_form.id).delete()
