...
def add_question(
    ...
    question = ReviewQuestion(
        review_form_id, question_id, type, is_required, order, weight
    )
...
def add_evaluation_question(
    form_id, order, headlines, descriptions, min_score, max_score
):
...
def add_information_question(form_id, order, en_headline, type="information"):
    ...
    headlines = {"en": en.headline, "fr": fr.headline}
    return add_question(
        form_id, type, False, order, question_id=question.id, headlines=headlines
    )
...
def upgrade():
    ...
    config = ReviewConfiguration(
        review_form_id=form.id, num_reviews_required=5, num_optional_reviews=0
    )
