...
"""
    ...
    op.execute(
        "UPDATE question SET description='Your university or company' WHERE headline='Affiliation' and application_form_id=2"
    )

    company_question = (
        session.query(Question)
        .filter_by(headline="Institution or Company", application_form_id=2)
        .first()
    )

    if company_question:
        op.execute(
            "DELETE FROM answer WHERE question_id={}".format(company_question.id)
        )
        op.execute("DELETE FROM question WHERE id={}".format(company_question.id))

    op.execute(
        "UPDATE question set description='The country where you work or study.' WHERE headline='Country' AND application_form_id=2"
    )
    op.execute(
        "UPDATE question set is_required=True WHERE headline='Extended abstract type' and application_form_id=2"
    )
    op.execute(
        "UPDATE question set is_required=True WHERE headline='Extended abstract' and application_form_id=2"
    )
    op.execute(
        """UPDATE question set options='[{"value": "TMLSS2018", "label": "TMLSS 2018"}, {"value": "EEML2019", "label": "EEML 2019"}, {"value": "TMLSS2018-EEML2019", "label": "Both"}, {"value": "none", "label": "Neither"}]' WHERE headline='Previous attendance' AND application_form_id=2"""
    )
